...
public class EventOutcome_NonAggressionPact extends EventOutcome
{
    public String fromCivTAG;
    public String toCivTag;
    
    public EventOutcome_NonAggressionPact(final String fromCivTAG, final String toCivTag) {
    ...
    public void updateCiv(final int iCivID, final int bonus_duration) {
        try {
            ...
            if (fromCivID > 0 && toCivID > 0 && !DiplomacyManager.isAtWar(fromCivID, toCivID)) {
                DiplomacyManager.addNonAggressionPact(fromCivID, toCivID);
            }
        }
