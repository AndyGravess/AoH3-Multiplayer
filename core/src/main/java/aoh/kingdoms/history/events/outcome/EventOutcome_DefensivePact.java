...
public class EventOutcome_DefensivePact extends EventOutcome
{
    public String fromCivTAG;
    public String toCivTag;
    
    public EventOutcome_DefensivePact(final String fromCivTAG, final String toCivTag) {
    ...
    public void updateCiv(final int iCivID, final int bonus_duration) {
        try {
            ...
            if (fromCivID > 0 && toCivID > 0 && !DiplomacyManager.isAtWar(fromCivID, toCivID)) {
                DiplomacyManager.addDefensivePact(fromCivID, toCivID);
            }
        }
