...
public class EventOutcome_Province_Population_All extends EventOutcome
{
    public int value;
    
    public EventOutcome_Province_Population_All(final int value) {
        this.value = value;
    }
    ...
    public void updateCiv(final int iCivID, final int bonus_duration) {
        try {
            for (int i = 0; i < Game.getCiv(iCivID).getNumOfProvinces(); ++i) {
                Game.getProvince(Game.getCiv(iCivID).getProvinceID(i)).setPopulationOfCivID(iCivID, Game.getProvince(Game.getCiv(iCivID).getProvinceID(i)).getPopulationOfCivID(iCivID) + this.value);
            }
        }
