...
public class EventOutcome_Province_Unrest_All extends EventOutcome
{
    public float value;
    
    public EventOutcome_Province_Unrest_All(final float value) {
        this.value = value;
    }
    ...
    public void updateCiv(final int iCivID, final int bonus_duration) {
        try {
            for (int i = 0; i < Game.getCiv(iCivID).getNumOfProvinces(); ++i) {
                Game.getProvince(Game.getCiv(iCivID).getProvinceID(i)).setRevulutionaryRisk(Game.getProvince(Game.getCiv(iCivID).getProvinceID(i)).getRevulutionaryRisk() + this.value);
            }
        }
