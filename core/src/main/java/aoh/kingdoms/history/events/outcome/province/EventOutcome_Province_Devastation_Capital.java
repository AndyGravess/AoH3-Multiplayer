...
public class EventOutcome_Province_Devastation_Capital extends EventOutcome
{
    public float value;
    
    public EventOutcome_Province_Devastation_Capital(final float value) {
        this.value = value;
    }
    ...
    public void updateCiv(final int iCivID, final int bonus_duration) {
        try {
            Game.getProvince(Game.getCiv(iCivID).getCapitalProvinceID()).setDevastation(Game.getProvince(Game.getCiv(iCivID).getCapitalProvinceID()).getDevastation() + this.value / 100.0f);
        }
