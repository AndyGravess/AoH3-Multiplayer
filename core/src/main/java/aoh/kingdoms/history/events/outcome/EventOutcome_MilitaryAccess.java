...
public class EventOutcome_MilitaryAccess extends EventOutcome
{
    public String fromCivTAG;
    public String toCivTag;
    
    public EventOutcome_MilitaryAccess(final String fromCivTAG, final String toCivTag) {
    ...
    public void updateCiv(final int iCivID, final int bonus_duration) {
        try {
            ...
            if (fromCivID > 0 && toCivID > 0 && !DiplomacyManager.isAtWar(fromCivID, toCivID)) {
                DiplomacyManager.addMilitaryAccess(fromCivID, toCivID);
            }
        }
