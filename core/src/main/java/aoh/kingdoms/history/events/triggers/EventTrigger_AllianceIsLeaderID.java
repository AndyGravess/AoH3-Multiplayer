...
public class EventTrigger_AllianceIsLeaderID extends EventTrigger_Value
{
    public int value;
    
    public EventTrigger_AllianceIsLeaderID(final int value) {
        this.value = value;
    }
    ...
    public boolean outCondition(final int iCivID, final int iProvinceID) {
        if (Game.getCiv(iCivID).inAllianceSize > 0) {
            for (int i = 0; i < Game.getCiv(iCivID).inAllianceSize; ++i) {
                if (Game.getCiv(iCivID).inAlliance.get(i) == this.value && Game.alliancesSpecial.get(Game.getCiv(iCivID).inAlliance.get(i)).iLeaderCivID == iCivID) {
                    return true;
                }
            }
        }
    ...
    public String getText3() {
        ...
        if (Game.getCiv(Game.player.iCivID).inAllianceSize > 0) {
            for (int i = 0; i < Game.getCiv(Game.player.iCivID).inAllianceSize; ++i) {
                if (Game.getCiv(Game.player.iCivID).inAlliance.get(i) == this.value && Game.alliancesSpecial.get(Game.getCiv(Game.player.iCivID).inAlliance.get(i)).iLeaderCivID == Game.player.iCivID) {
                    state = true;
                    break;
                }
            }
