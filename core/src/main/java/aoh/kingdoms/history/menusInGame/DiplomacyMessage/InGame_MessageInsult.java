...
public class InGame_MessageInsult extends Menu
{
    ...
    public InGame_MessageInsult(final String nKey) {
        ...
        InGame_MessageInsult.key = nKey;
        final PMessage pMessage = Game.player.getMessage(InGame_MessageInsult.key);
        if (pMessage != null) {
            InGame_MessageInsult.iCivID = pMessage.fromCivID;
            ...
            menuElements.add(new ButtonFlag_Diplomacy(InGame_MessageInsult.iCivID, menuWidth / 2 - maxWidth / 2 - CFG.PADDING * 2 - ImageManager.getImage(Images.flagDiplomacyOver).getWidth(), buttonY + tempTitlePaddingY, true));
            ...
            menuElements.add(new Text_Static_ID(InGame_MessageInsult.iCivID, Game.getCiv(InGame_MessageInsult.iCivID).getCivName(), CFG.FONT_REGULAR, -1, paddingLeft, buttonY, tempTextW, tempTitleH));
            ...
            menuElements.add(new ButtonStatsRectIMG_Diplomacy(Game.lang.get("Opinion") + ": " + CFG.getPrecision2(Game.getCiv(InGame_MessageInsult.iCivID).diplomacy.getRelation(Game.player.iCivID), 1), Images.relations, statsX, buttonY, statsW * 2 + CFG.PADDING / 2 * 2, statsH, maxIconW, 0) {
                @Override
                ...
                public void buildElementHover() {
                    this.menuElementHover = InGame_Civ.getHoverBetweenCivilizations(InGame_MessageInsult.iCivID, Game.player.iCivID, Game.getCiv(Game.player.iCivID).diplomacy.isImprovingRelations(InGame_MessageInsult.iCivID), Game.getCiv(Game.player.iCivID).diplomacy.isDamagingRelations(InGame_MessageInsult.iCivID));
                }
            });
            menuElements.add(new ButtonStatsRectIMG_Diplomacy("" + Game.getCiv(InGame_MessageInsult.iCivID).getNumOfProvinces(), Images.provinces, statsX, buttonY + CFG.PADDING + statsH, statsW, statsH, maxIconW, 0) {
                @Override
                public void buildElementHover() {
            ...
            menuElements.add(new ButtonStatsRectIMG_Diplomacy(Game_Calendar.getDate_ByTurnID(pMessage.expiresTurnID), Images.time, statsX, buttonY + (CFG.PADDING + statsH) * 2, statsW * 2 + CFG.PADDING, statsH, maxIconW, 0) {
                @Override
                public void buildElementHover() {
                    final List<MenuElement_HoverElement> nElements = new ArrayList<MenuElement_HoverElement>();
                    final List<MenuElement_HoverElement_Type> nData = new ArrayList<MenuElement_HoverElement_Type>();
            ...
            RulersManager.loadRulerIMG_DiplomacyLeft(InGame_MessageInsult.iCivID);
            RulersManager.loadRulerIMG_DiplomacyRight(Game.player.iCivID);
            menuElements.add(new ButtonRuler_Diplomacy(InGame_MessageInsult.iCivID, buttonX + CFG.PADDING, buttonY));
            ...
            menuElements.add(new ButtonGame(Game.lang.get("WithdrawTheDiplomat"), CFG.FONT_REGULAR, -1, paddingLeft, buttonY, (menuWidth - paddingLeft * 2 - CFG.PADDING / 2 * 2) / 2, true) {
                @Override
                public void actionElement() {
                    InGame_MessageInsult.confirm();
                }
            });
            menuElements.add(new ButtonGame(Game.lang.get("DeclareWar"), CFG.FONT_REGULAR, -1, paddingLeft + CFG.PADDING + (menuWidth - paddingLeft * 2 - CFG.PADDING / 2 * 2) / 2, buttonY, (menuWidth - paddingLeft * 2 - CFG.PADDING / 2 * 2) / 2, true) {
                @Override
                public void actionElement() {
                    final PMessage tMessage = Game.player.getMessage(InGame_MessageInsult.key);
                    if (tMessage != null) {
                        tMessage.onAccept();
                        InGame_DeclareWar.actionDeclareWar(Game.player.iCivID, tMessage.fromCivID, true);
                    }
                    Game.menuManager.setVisibleInGame_PopUp(false);
                    Game.player.removeMessage(InGame_MessageInsult.key);
                }
        ...
        this.initMenu(new MenuTitleIMG_Message(Game.lang.get("AnInsultFrom") + ": " + Game.getCiv(InGame_MessageInsult.iCivID).getCivName(), true, false, Images.title600) {
            @Override
            public long getTime() {
                return InGame_MessageInsult.lTime;
            }
        ...
        InGame_MessageInsult.lTime = CFG.currentTimeMillis;
    }
    ...
    public void draw(final SpriteBatch oSB, final int iTranslateX, int iTranslateY, final boolean menuIsActive, final Status titleStatus) {
        ...
        if (InGame_MessageInsult.lTime + 60L >= CFG.currentTimeMillis) {
            iTranslateY = iTranslateY - CFG.BUTTON_HEIGHT * 3 / 5 + (int)(CFG.BUTTON_HEIGHT * 3 / 5 * ((CFG.currentTimeMillis - InGame_MessageInsult.lTime) / 60.0f));
        }
    ...
    static {
        InGame_MessageInsult.lTime = 0L;
        InGame_MessageInsult.iCivID = 0;
    }
}
