...
public class InGame_BudgetIncomeTaxation extends Menu
{
    ...
    public InGame_BudgetIncomeTaxation() {
        ...
        menuElements.add(new ButtonStatsBudget_Right2("+" + CFG.getPrecision2(Game.getCiv(Game.player.iCivID).fTotalIncomePerMonth + Game.getCiv(this.iActiveCivID).civBonuses.MonthlyIncome, 100), paddingLeft + tButtonW + CFG.PADDING, buttonY, tButtonRightW, tButtonH) {
            ...
            public String getTextToDraw() {
                if (this.lastValue != Game.getCiv(Game.player.iCivID).fTotalIncomePerMonth + Game.getCiv(InGame_BudgetIncomeTaxation.this.iActiveCivID).civBonuses.MonthlyIncome) {
                    final float fVal = Game.getCiv(Game.player.iCivID).fTotalIncomePerMonth + Game.getCiv(InGame_BudgetIncomeTaxation.this.iActiveCivID).civBonuses.MonthlyIncome;
        ...
        menuElements.add(new Text_TitleBlueSort(InGame_BudgetIncomeTaxation.iSortID == 0 || InGame_BudgetIncomeTaxation.iSortID == 1, InGame_BudgetIncomeTaxation.iSortID == 1, Game.lang.get("Name"), -1, buttonX, buttonY, r0W, CFG.TEXT_HEIGHT + CFG.PADDING * 6, CFG.FONT_BOLD_SMALL) {
            @Override
            ...
            @Override
            public void actionElement() {
                if (InGame_BudgetIncomeTaxation.iSortID == 0) {
                    InGame_BudgetIncomeTaxation.iSortID = 1;
                }
                else {
                    InGame_BudgetIncomeTaxation.iSortID = 0;
                }
                Game.menuManager.rebuildInGame_BudgetIncomeTaxation();
                InGame_BudgetIncomeTaxation.lTime = 0L;
            }
        ...
        menuElements.add(new Text_TitleBlueSort(InGame_BudgetIncomeTaxation.iSortID == 2 || InGame_BudgetIncomeTaxation.iSortID == 3, InGame_BudgetIncomeTaxation.iSortID == 3, Game.lang.get("Population"), -1, buttonX, buttonY, r0W, CFG.TEXT_HEIGHT + CFG.PADDING * 6, CFG.FONT_BOLD_SMALL) {
            @Override
            public void drawLines(final SpriteBatch oSB, final int iTranslateX, final int iTranslateY, final boolean isActive, final boolean scrollableY) {
            }
            ...
            @Override
            public void actionElement() {
                if (InGame_BudgetIncomeTaxation.iSortID == 2) {
                    InGame_BudgetIncomeTaxation.iSortID = 3;
                }
                else {
                    InGame_BudgetIncomeTaxation.iSortID = 2;
                }
                Game.menuManager.rebuildInGame_BudgetIncomeTaxation();
                InGame_BudgetIncomeTaxation.lTime = 0L;
            }
        ...
        buttonX += menuElements.get(menuElements.size() - 1).getWidth();
        menuElements.add(new Text_TitleBlueSort(InGame_BudgetIncomeTaxation.iSortID == 4 || InGame_BudgetIncomeTaxation.iSortID == 5, InGame_BudgetIncomeTaxation.iSortID == 5, Game.lang.get("TaxEfficiency"), -1, buttonX, buttonY, r1W, CFG.TEXT_HEIGHT + CFG.PADDING * 6, CFG.FONT_BOLD_SMALL) {
            @Override
            public void drawLines(final SpriteBatch oSB, final int iTranslateX, final int iTranslateY, final boolean isActive, final boolean scrollableY) {
            }
            ...
            @Override
            public void actionElement() {
                if (InGame_BudgetIncomeTaxation.iSortID == 4) {
                    InGame_BudgetIncomeTaxation.iSortID = 5;
                }
                else {
                    InGame_BudgetIncomeTaxation.iSortID = 4;
                }
                Game.menuManager.rebuildInGame_BudgetIncomeTaxation();
                InGame_BudgetIncomeTaxation.lTime = 0L;
            }
        ...
        buttonX += menuElements.get(menuElements.size() - 1).getWidth();
        menuElements.add(new Text_TitleBlueSort(InGame_BudgetIncomeTaxation.iSortID == 6 || InGame_BudgetIncomeTaxation.iSortID == 7, InGame_BudgetIncomeTaxation.iSortID == 7, Game.lang.get("IncomeTaxation"), -1, buttonX, buttonY, r1W, CFG.TEXT_HEIGHT + CFG.PADDING * 6, CFG.FONT_BOLD_SMALL) {
            @Override
            public void drawLines(final SpriteBatch oSB, final int iTranslateX, final int iTranslateY, final boolean isActive, final boolean scrollableY) {
            }
            ...
            @Override
            public void actionElement() {
                if (InGame_BudgetIncomeTaxation.iSortID == 6) {
                    InGame_BudgetIncomeTaxation.iSortID = 7;
                }
                else {
                    InGame_BudgetIncomeTaxation.iSortID = 6;
                }
                Game.menuManager.rebuildInGame_BudgetIncomeTaxation();
                InGame_BudgetIncomeTaxation.lTime = 0L;
            }
            ...
            public void buildElementHover() {
                ...
                final List<MenuElement_HoverElement_Type> nData = new ArrayList<MenuElement_HoverElement_Type>();
                nData.add(new MenuElement_HoverElement_Type_TextTitle(Game.lang.get("SortBy") + ": "));
                nData.add(new MenuElement_HoverElement_Type_TextTitle(Game.lang.get("IncomeTaxation"), Colors.HOVER_GOLD));
                nElements.add(new MenuElement_HoverElement(nData));
                nData.clear();
                this.menuElementHover = new MenuElement_Hover(nElements);
            }
        ...
        while (tProvinces.size() > 0) {
            ...
            else if (InGame_BudgetIncomeTaxation.iSortID == 1) {
                for (int o = 1; o < tProvinces.size(); ++o) {
            ...
            else if (InGame_BudgetIncomeTaxation.iSortID == 2) {
                for (int o = 1; o < tProvinces.size(); ++o) {
            ...
            else if (InGame_BudgetIncomeTaxation.iSortID == 3) {
                for (int o = 1; o < tProvinces.size(); ++o) {
                    if (Game.getProvince(tProvinces.get(o)).getPopulationTotal() < Game.getProvince(tProvinces.get(toAddID)).getPopulationTotal()) {
                        toAddID = o;
                    }
            ...
            else if (InGame_BudgetIncomeTaxation.iSortID == 4) {
                for (int o = 1; o < tProvinces.size(); ++o) {
            ...
            else if (InGame_BudgetIncomeTaxation.iSortID == 5) {
                for (int o = 1; o < tProvinces.size(); ++o) {
                    if (Game.getProvince(tProvinces.get(o)).getTaxEfficiencyWithBonuses() < Game.getProvince(tProvinces.get(toAddID)).getTaxEfficiencyWithBonuses()) {
                        toAddID = o;
                    }
            ...
            else if (InGame_BudgetIncomeTaxation.iSortID == 6) {
                for (int o = 1; o < tProvinces.size(); ++o) {
                    if (Game.getIncomePopulationTaxation(tProvinces.get(toAddID)) < Game.getIncomePopulationTaxation(tProvinces.get(o))) {
                        toAddID = o;
                    }
                }
            }
            else if (InGame_BudgetIncomeTaxation.iSortID == 7) {
                for (int o = 1; o < tProvinces.size(); ++o) {
                    if (Game.getIncomePopulationTaxation(tProvinces.get(toAddID)) > Game.getIncomePopulationTaxation(tProvinces.get(o))) {
                        toAddID = o;
                    }
            ...
            menuElements.add(new Text_StaticBG_TaxEfficiency("" + CFG.getPrecision2(Game.getProvince(tProvinces.get(toAddID)).getTaxEfficiencyWithBonuses(), 100) + "%", CFG.FONT_REGULAR_SMALL, -1, buttonX, buttonY, r1W, buttonH, (int)tProvinces.get(toAddID)) {
                ...
                public void actionElement() {
                    if (Game.getProvince(this.getCurrent()).getCivID() == Game.player.iCivID) {
                        ...
                        else if (Game.getCiv(Game.player.iCivID).fLegacy < Game.getIncreaseTaxEfficiencyCostLegacy(this.getCurrent())) {
                            Game.menuManager.addToastInsufficient(Game.lang.get("InsufficientLegacy") + ": ", CFG.getPrecision2(Game.getIncreaseTaxEfficiencyCostLegacy(this.getCurrent()), 100), Images.legacy);
                        }
                        else if (!Game.getProvince(this.getCurrent()).addIncreaseTaxEfficiencyInProvince()) {
                            Game.menuManager.addToastInsufficient(Game.lang.get("InsufficientGold") + ": ", CFG.getPrecision2(Game.getIncreaseTaxEfficiencyCost(this.getCurrent()), 100), Images.gold);
                        }
                        else {
                            ProvinceDraw.addProvinceDot_TaxEfficiency(this.getCurrent());
            ...
            buttonX += menuElements.get(menuElements.size() - 1).getWidth() + CFG.PADDING;
            fGold = Game.getIncomePopulationTaxation(tProvinces.get(toAddID));
            menuElements.add(new ButtonStatsBudget_Right2(((fGold > 0.0f) ? "+" : "") + CFG.getPrecision2(fGold, 100), buttonX, buttonY, r1W, buttonH, (int)tProvinces.get(toAddID)) {
                @Override
                public String getTextToDraw() {
                    if (this.lastValue != Game.getIncomePopulationTaxation(this.id)) {
                        final float fVal = Game.getIncomePopulationTaxation(this.id);
                        this.setText(((fVal > 0.0f) ? "+" : "") + CFG.getPrecision2(fVal, 100));
                        this.lastValue = fVal;
                        if (fVal == 0.0f) {
                            this.nColor = Colors.COLOR_TEXT_MODIFIER_NEUTRAL;
                ...
                public void buildElementHover() {
                    final List<MenuElement_HoverElement> nElements = new ArrayList<MenuElement_HoverElement>();
                    final List<MenuElement_HoverElement_Type> nData = new ArrayList<MenuElement_HoverElement_Type>();
                    final float fGold = Game.getIncomePopulationTaxation(this.id);
                    nData.add(new MenuElement_HoverElement_Type_TextTitle(Game.lang.get("MonthlyIncomeTaxation") + ": ", CFG.FONT_BOLD));
        ...
        this.initMenu(new MenuTitleIMG_FlagCenter(Game.lang.get("IncomeTaxation"), Game.getCiv(this.iActiveCivID).getCivName(), false, false, Images.title500) {
            @Override
            public int getFlagCivID() {
                return InGame_BudgetIncomeTaxation.this.iActiveCivID;
            }
            
            @Override
            public long getTime() {
                return InGame_BudgetIncomeTaxation.lTime2;
            }
    ...
    public void draw(final SpriteBatch oSB, int iTranslateX, final int iTranslateY, final boolean menuIsActive, final Status titleStatus) {
        if (InGame_BudgetIncomeTaxation.lTime + 60L >= CFG.currentTimeMillis) {
            iTranslateX = iTranslateX - CFG.BUTTON_WIDTH + (int)(CFG.BUTTON_WIDTH * ((CFG.currentTimeMillis - InGame_BudgetIncomeTaxation.lTime) / 60.0f));
        }
    ...
    public void updateLanguage() {
        ...
        this.getTitle().setText(Game.lang.get("MonthlyIncomeTaxation"));
    }
    
    @Override
    public void setVisible(final boolean visible) {
        super.setVisible(visible);
        InGame_BudgetIncomeTaxation.lTime2 = CFG.currentTimeMillis;
    }
    
    static {
        InGame_BudgetIncomeTaxation.lTime = 0L;
        InGame_BudgetIncomeTaxation.lTime2 = 0L;
        InGame_BudgetIncomeTaxation.iSortID = 6;
    }
}
