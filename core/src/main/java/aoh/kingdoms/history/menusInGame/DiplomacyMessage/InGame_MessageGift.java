...
public class InGame_MessageGift extends Menu
{
    ...
    public InGame_MessageGift(final String nKey) {
        ...
        InGame_MessageGift.key = nKey;
        final PMessage pMessage = Game.player.getMessage(InGame_MessageGift.key);
        if (pMessage != null) {
            InGame_MessageGift.iCivID = pMessage.fromCivID;
            ...
            menuElements.add(new ButtonFlag_Diplomacy(InGame_MessageGift.iCivID, menuWidth / 2 - maxWidth / 2 - CFG.PADDING * 2 - ImageManager.getImage(Images.flagDiplomacyOver).getWidth(), buttonY + tempTitlePaddingY, true));
            ...
            menuElements.add(new Text_Static_ID(InGame_MessageGift.iCivID, Game.getCiv(InGame_MessageGift.iCivID).getCivName(), CFG.FONT_REGULAR, -1, paddingLeft, buttonY, tempTextW, tempTitleH));
            ...
            menuElements.add(new ButtonStatsRectIMG_Diplomacy(Game.lang.get("Opinion") + ": " + CFG.getPrecision2(Game.getCiv(InGame_MessageGift.iCivID).diplomacy.getRelation(Game.player.iCivID), 1), Images.relations, statsX, buttonY, statsW * 2 + CFG.PADDING / 2 * 2, statsH, maxIconW, 0) {
                @Override
                ...
                public void buildElementHover() {
                    this.menuElementHover = InGame_Civ.getHoverBetweenCivilizations(InGame_MessageGift.iCivID, Game.player.iCivID, Game.getCiv(Game.player.iCivID).diplomacy.isImprovingRelations(InGame_MessageGift.iCivID), Game.getCiv(Game.player.iCivID).diplomacy.isDamagingRelations(InGame_MessageGift.iCivID));
                }
            });
            menuElements.add(new ButtonStatsRectIMG_Diplomacy("" + Game.getCiv(InGame_MessageGift.iCivID).getNumOfProvinces(), Images.provinces, statsX, buttonY + CFG.PADDING + statsH, statsW, statsH, maxIconW, 0) {
                @Override
                public void buildElementHover() {
            ...
            menuElements.add(new ButtonStatsRectIMG_Diplomacy(CFG.getNumberWithSpaces("" + pMessage.iValue1), Images.gold, statsX, buttonY + (CFG.PADDING + statsH) * 2, statsW * 2 + CFG.PADDING, statsH, maxIconW, 0) {
                @Override
                public void buildElementHover() {
                    final List<MenuElement_HoverElement> nElements = new ArrayList<MenuElement_HoverElement>();
                    final List<MenuElement_HoverElement_Type> nData = new ArrayList<MenuElement_HoverElement_Type>();
                    nData.add(new MenuElement_HoverElement_Type_TextTitle(Game.lang.get("Gold") + ": "));
                    nData.add(new MenuElement_HoverElement_Type_TextTitle(this.getText(), CFG.FONT_BOLD, Colors.HOVER_GOLD));
                    nData.add(new MenuElement_HoverElement_Type_ImageTitle(Images.goldPositive, CFG.PADDING, 0));
                    nElements.add(new MenuElement_HoverElement(nData));
                    nData.clear();
                    this.menuElementHover = new MenuElement_Hover(nElements);
                }
            });
            RulersManager.loadRulerIMG_DiplomacyLeft(InGame_MessageGift.iCivID);
            RulersManager.loadRulerIMG_DiplomacyRight(Game.player.iCivID);
            menuElements.add(new ButtonRuler_Diplomacy(InGame_MessageGift.iCivID, buttonX + CFG.PADDING, buttonY));
            ...
            menuElements.add(new ButtonGame(Game.lang.get("Refuse"), CFG.FONT_REGULAR, -1, paddingLeft, buttonY, (menuWidth - paddingLeft * 2 - CFG.PADDING / 2 * 2) / 2, true) {
                ...
                public void actionElement() {
                    final PMessage tMessage = Game.player.getMessage(InGame_MessageGift.key);
                    if (tMessage != null) {
                        tMessage.onRefuse();
                    }
                    Game.menuManager.setVisibleInGame_PopUp(false);
                    Game.player.removeMessage(InGame_MessageGift.key);
                }
            });
        ...
        this.initMenu(new MenuTitleIMG_Message(Game.lang.get("AGiftFromCivA", Game.getCiv(InGame_MessageGift.iCivID).getCivName()), true, false, Images.title600) {
            @Override
            public long getTime() {
                return InGame_MessageGift.lTime;
            }
        ...
        InGame_MessageGift.lTime = CFG.currentTimeMillis;
    }
    ...
    public void draw(final SpriteBatch oSB, final int iTranslateX, int iTranslateY, final boolean menuIsActive, final Status titleStatus) {
        ...
        if (InGame_MessageGift.lTime + 60L >= CFG.currentTimeMillis) {
            iTranslateY = iTranslateY - CFG.BUTTON_HEIGHT * 3 / 5 + (int)(CFG.BUTTON_HEIGHT * 3 / 5 * ((CFG.currentTimeMillis - InGame_MessageGift.lTime) / 60.0f));
        }
    ...
    static {
        InGame_MessageGift.lTime = 0L;
        InGame_MessageGift.iCivID = 0;
    }
}
