...
public class InGame_Alliance extends Menu
{
    public static final int ANIMATION_TIME = 60;
    public static long lTime;
    public static int iCivID;
    
    public InGame_Alliance(final int nCivID) {
        ...
        menuElements.add(new ButtonFlag_Diplomacy(InGame_Alliance.iCivID, menuWidth / 2 + maxWidth / 2 + CFG.PADDING * 2, buttonY + tempTitlePaddingY, true));
        menuElements.add(new Text_Static_ID(InGame_Alliance.iCivID, Game.getCiv(InGame_Alliance.iCivID).getCivName(), CFG.FONT_REGULAR, -1, menuWidth / 2 + maxWidth / 2 + CFG.PADDING * 2 + ImageManager.getImage(Images.flagDiplomacyOver).getWidth(), buttonY, tempTextW, tempTitleH));
        ...
        menuElements.add(new TextIcon_Diplomacy(Images.allianceBig, paddingLeft, buttonY, menuWidth - paddingLeft * 2, tempTitleH, maxWidth) {
            @Override
        ...
        menuElements.add(new ButtonStatsRectIMG_Diplomacy(Game.lang.get("Opinion") + ": " + CFG.getPrecision2(Game.getCiv(InGame_Alliance.iCivID).diplomacy.getRelation(Game.player.iCivID), 1), Images.relations, statsX, buttonY, statsW * 2 + CFG.PADDING / 2 * 2, statsH, maxIconW, 0) {
            @Override
            ...
            public void buildElementHover() {
                this.menuElementHover = InGame_Civ.getHoverBetweenCivilizations(InGame_Alliance.iCivID, Game.player.iCivID, Game.getCiv(Game.player.iCivID).diplomacy.isImprovingRelations(InGame_Alliance.iCivID), Game.getCiv(Game.player.iCivID).diplomacy.isDamagingRelations(InGame_Alliance.iCivID));
            }
        });
        ...
        menuElements.add(new ButtonStatsRectIMG_Diplomacy_Flip("" + CFG.getShortNumber(Game.getCiv(InGame_Alliance.iCivID).iRegimentsLimit), Images.regimentsLimit, statsX + CFG.PADDING + statsW, buttonY + CFG.PADDING + statsH, statsW, statsH, maxIconW, 0) {
            @Override
            public void buildElementHover() {
                final List<MenuElement_HoverElement> nElements = new ArrayList<MenuElement_HoverElement>();
                final List<MenuElement_HoverElement_Type> nData = new ArrayList<MenuElement_HoverElement_Type>();
                nData.add(new MenuElement_HoverElement_Type_TextTitle(Game.lang.get("RegimentsLimit") + ": "));
                nData.add(new MenuElement_HoverElement_Type_TextTitle("" + Game.getCiv(InGame_Alliance.iCivID).iRegimentsLimit, CFG.FONT_BOLD, Colors.HOVER_GOLD));
                nData.add(new MenuElement_HoverElement_Type_ImageTitle(Images.regimentsLimit, CFG.PADDING, 0));
                nElements.add(new MenuElement_HoverElement(nData));
                nData.clear();
                this.menuElementHover = new MenuElement_Hover(nElements);
        ...
        menuElements.add(new ButtonStatsRectIMG_Diplomacy_Flip("" + Game.getCiv(InGame_Alliance.iCivID).getNumOfProvinces(), Images.provinces, statsX + CFG.PADDING + statsW, buttonY + (CFG.PADDING + statsH) * 2, statsW, statsH, maxIconW, 0) {
            @Override
            public void buildElementHover() {
                final List<MenuElement_HoverElement> nElements = new ArrayList<MenuElement_HoverElement>();
                final List<MenuElement_HoverElement_Type> nData = new ArrayList<MenuElement_HoverElement_Type>();
        ...
        RulersManager.loadRulerIMG_DiplomacyRight(InGame_Alliance.iCivID);
        menuElements.add(new ButtonRuler_Diplomacy(Game.player.iCivID, buttonX + CFG.PADDING, buttonY));
        menuElements.add(new ButtonRuler_Diplomacy(InGame_Alliance.iCivID, menuWidth - paddingLeft - ButtonRuler_Diplomacy.getButtonWidth() - CFG.PADDING, buttonY) {
            @Override
        ...
        menuElements.add(new TextBonus(Game.lang.get("Cost") + ": ", "" + CFG.getPrecision2(GameValues.diplomacy.DIPLOMACY_ALLIANCE_COST, 100), Images.diplomacy, paddingLeft + CFG.PADDING + (menuWidth - paddingLeft * 2 - CFG.PADDING / 2 * 2) / 2, buttonY, (menuWidth - paddingLeft * 2 - CFG.PADDING / 2 * 2) / 2, CFG.TEXT_HEIGHT + CFG.PADDING * 5, iconWidth) {
            @Override
            public void buildElementHover() {
                final List<MenuElement_HoverElement> nElements = new ArrayList<MenuElement_HoverElement>();
                final List<MenuElement_HoverElement_Type> nData = new ArrayList<MenuElement_HoverElement_Type>();
                nData.add(new MenuElement_HoverElement_Type_Button_TextBonus(Game.lang.get("DiplomacyPoints") + ": ", "" + CFG.getPrecision2(GameValues.diplomacy.DIPLOMACY_ALLIANCE_COST, 100), Images.diplomacy, CFG.FONT_REGULAR_SMALL, CFG.FONT_BOLD_SMALL, Colors.HOVER_LEFT, Colors.HOVER_GOLD));
                nElements.add(new MenuElement_HoverElement(nData));
                nData.clear();
                this.menuElementHover = new MenuElement_Hover(nElements);
            }
        ...
        buttonY += menuElements.get(menuElements.size() - 1).getHeight() + CFG.PADDING;
        final int score = DiplomacyManager.getAlliance_Score(Game.player.iCivID, InGame_Alliance.iCivID);
        menuElements.add(new Button_Likelihood(Game.lang.get("Score") + ": ", "" + score, buttonX, buttonY, menuWidth - paddingLeft * 2, CFG.BUTTON_HEIGHT / 2, (score > 0) ? (0.5f + Math.min(0.49f, Math.abs(score) / 100.0f)) : (0.49f - Math.min(0.48f, Math.abs(score) / 100.0f)), (score > 0) ? Images.v : Images.x) {
            @Override
            public void buildElementHover() {
                this.menuElementHover = InGame_Alliance.getHover();
            }
        });
        buttonY += menuElements.get(menuElements.size() - 1).getHeight() + CFG.PADDING;
        if (score < 0) {
            ...
            buttonY += menuElements.get(menuElements.size() - 1).getHeight() + CFG.PADDING;
            if (Game.getCiv(Game.player.iCivID).diplomacy.alliance.size() >= DiplomacyManager.getMaxNumberOfAlliances(Game.player.iCivID) || Game.getCiv(InGame_Alliance.iCivID).diplomacy.alliance.size() >= DiplomacyManager.getMaxNumberOfAlliances(InGame_Alliance.iCivID)) {
                ...
                if (Game.getCiv(InGame_Alliance.iCivID).diplomacy.alliance.size() >= DiplomacyManager.getMaxNumberOfAlliances(InGame_Alliance.iCivID)) {
                    menuElements.add(new ButtonStatsRectIMG_Bonuses(Game.getCiv(InGame_Alliance.iCivID).getCivName() + ", " + Game.lang.get("MaxNumOfAlliances") + ": ", "" + Game.getCiv(InGame_Alliance.iCivID).diplomacy.alliance.size() + " / " + DiplomacyManager.getMaxNumberOfAlliances(InGame_Alliance.iCivID), Images.alliance, paddingLeft, buttonY, menuWidth - paddingLeft * 2, CFG.BUTTON_HEIGHT3, ImageManager.getImage(Images.alliance).getWidth(), CFG.FONT_REGULAR_SMALL, CFG.FONT_BOLD_SMALL) {
                        @Override
                        public Color getColorBonus() {
                            return Colors.getColorNegative(false, this.getIsHovered());
                        }
        ...
        menuElements.add(new ButtonGame_ImageSparks(Game.lang.get("SendProposal"), CFG.FONT_REGULAR, -1, paddingLeft + CFG.PADDING + (menuWidth - paddingLeft * 2 - CFG.PADDING / 2 * 2) / 2, buttonY, (menuWidth - paddingLeft * 2 - CFG.PADDING / 2 * 2) / 2, true, Images.alliance) {
            @Override
            public void actionElement() {
            ...
            public void buildElementHover() {
                ...
                nData.add(new MenuElement_HoverElement_Type_Text("-" + CFG.getPrecision2(GameValues.diplomacy.DIPLOMACY_ALLIANCE_COST, 100), CFG.FONT_BOLD_SMALL, Colors.HOVER_NEGATIVE));
        ...
        this.initMenu(new MenuTitleIMG(Game.lang.get("OfferAlliance"), true, false, Images.title600) {
            @Override
            public long getTime() {
                return InGame_Alliance.lTime;
            }
    ...
    public void draw(final SpriteBatch oSB, final int iTranslateX, int iTranslateY, final boolean menuIsActive, final Status titleStatus) {
        ...
        if (InGame_Alliance.lTime + 60L >= CFG.currentTimeMillis) {
            iTranslateY = iTranslateY - CFG.BUTTON_HEIGHT * 3 / 5 + (int)(CFG.BUTTON_HEIGHT * 3 / 5 * ((CFG.currentTimeMillis - InGame_Alliance.lTime) / 60.0f));
        }
    ...
    public void setVisible(final boolean visible) {
        ...
        InGame_Alliance.lTime = CFG.currentTimeMillis;
        DiplomacyManager.updateAnimationTime();
    }
    
    public static void confirm() {
        if (Game.getCiv(Game.player.iCivID).fDiplomacy < GameValues.diplomacy.DIPLOMACY_ALLIANCE_COST) {
            Game.menuManager.addToastInsufficient(Game.lang.get("Cost") + ", " + Game.lang.get("DiplomacyPoints") + ": ", CFG.getPrecision2(GameValues.diplomacy.DIPLOMACY_ALLIANCE_COST, 100), Images.diplomacy);
            return;
        }
        DiplomacyManager.offerAlliance(Game.player.iCivID, InGame_Alliance.iCivID);
        Game.menuManager.setVisibleInGame_PopUp(false);
        ProvinceDraw.addDiplomacyLines(Game.getCiv(Game.player.iCivID).getCapitalProvinceID(), Game.getCiv(InGame_Alliance.iCivID).getCapitalProvinceID(), DiplomacyManager.COLOR_ALLIANCE);
    }
    ...
    public static MenuElement_Hover getHover() {
        ...
        nElements.add(new MenuElement_HoverElement(nData));
        nData.clear();
        nData.add(new MenuElement_HoverElement_Type_Button_TextBonus(Game.lang.get("Score") + ": ", "" + score, Images.alliance, CFG.FONT_REGULAR_SMALL, CFG.FONT_BOLD_SMALL, Colors.HOVER_LEFT, Colors.HOVER_GOLD));
        nElements.add(new MenuElement_HoverElement(nData));
        nData.clear();
        ...
        nData.clear();
        nData.add(new MenuElement_HoverElement_Type_Text(Game.lang.get("MaxNumOfAlliances") + ": ", CFG.FONT_REGULAR_SMALL, Colors.HOVER_LEFT));
        nData.add(new MenuElement_HoverElement_Type_Text("" + Game.getCiv(InGame_Alliance.iCivID).diplomacy.alliance.size() + " / " + DiplomacyManager.getMaxNumberOfAlliances(InGame_Alliance.iCivID), CFG.FONT_BOLD_SMALL, (DiplomacyManager.getMaxNumberOfAlliances(InGame_Alliance.iCivID) <= Game.getCiv(InGame_Alliance.iCivID).diplomacy.alliance.size()) ? Colors.HOVER_NEGATIVE : Colors.HOVER_GOLD));
        nData.add(new MenuElement_HoverElement_Type_Flag(InGame_Alliance.iCivID, CFG.PADDING, 0));
        nData.add(new MenuElement_HoverElement_Type_Image(Images.alliance, CFG.PADDING, 0));
        nElements.add(new MenuElement_HoverElement(nData));
        nData.clear();
        nData.add(new MenuElement_HoverElement_Type_Line());
        ...
        nData.add(new MenuElement_HoverElement_Type_Text("" + CFG.getPrecision2(DiplomacyManager.getAlliance_Score_BASE_VALUE(Game.player.iCivID, InGame_Alliance.iCivID), 100), CFG.FONT_BOLD_SMALL, Colors.HOVER_GOLD));
        nElements.add(new MenuElement_HoverElement(nData));
        nData.clear();
        nData.add(new MenuElement_HoverElement_Type_Text(Game.lang.get("Distance") + ": ", CFG.FONT_REGULAR_SMALL, Colors.HOVER_LEFT));
        nData.add(new MenuElement_HoverElement_Type_Text("" + CFG.getPrecision2(GameValues.diplomacy.ALLIANCE_SCORE_DISTANCE * Game.getDistance_PercOfMax(Game.getCiv(Game.player.iCivID).getCapitalProvinceID(), Game.getCiv(InGame_Alliance.iCivID).getCapitalProvinceID()), 100), CFG.FONT_BOLD_SMALL, Colors.HOVER_GOLD));
        nElements.add(new MenuElement_HoverElement(nData));
        nData.clear();
        if (Game.getCiv(Game.player.iCivID).getIdeologyID() != Game.getCiv(InGame_Alliance.iCivID).getIdeologyID()) {
            nData.add(new MenuElement_HoverElement_Type_Text(Game.lang.get("Government") + ": ", CFG.FONT_REGULAR_SMALL, Colors.HOVER_LEFT));
            if (Game.ideologiesManager.getIdeology(Game.getCiv(Game.player.iCivID).getIdeologyID()).GOV_GROUP_ID != Game.ideologiesManager.getIdeology(Game.getCiv(InGame_Alliance.iCivID).getIdeologyID()).GOV_GROUP_ID) {
        ...
        if (Game.getCiv(Game.player.iCivID).getReligionID() != Game.getCiv(InGame_Alliance.iCivID).getReligionID()) {
            nData.add(new MenuElement_HoverElement_Type_Text(Game.lang.get("Religion") + ": ", CFG.FONT_REGULAR_SMALL, Colors.HOVER_LEFT));
            if (Game.religionManager.getReligion(Game.getCiv(Game.player.iCivID).getReligionID()).ReligionGroupID != Game.religionManager.getReligion(Game.getCiv(InGame_Alliance.iCivID).getReligionID()).ReligionGroupID) {
        ...
        if (Game.getCiv(InGame_Alliance.iCivID).getPuppetOfCivID() == Game.player.iCivID) {
        ...
        nData.add(new MenuElement_HoverElement_Type_Text("" + CFG.getPrecision2(Game.getCiv(InGame_Alliance.iCivID).diplomacy.getRelation(Game.player.iCivID) * GameValues.diplomacy.ALLIANCE_SCORE_PER_RELATION, 10), CFG.FONT_BOLD_SMALL, Colors.HOVER_GOLD));
        nData.add(new MenuElement_HoverElement_Type_Image(Images.relations, CFG.PADDING, 0));
        nElements.add(new MenuElement_HoverElement(nData));
        nData.clear();
        float tScore = DiplomacyManager.getAlliance_Score_SameRivals(Game.player.iCivID, InGame_Alliance.iCivID);
        if (tScore > 0.0f) {
        ...
        if (Game.getCiv(Game.player.iCivID).diplomacy.isRival(InGame_Alliance.iCivID)) {
            nData.add(new MenuElement_HoverElement_Type_Text(Game.lang.get("Rival") + ": ", CFG.FONT_REGULAR_SMALL, Colors.HOVER_LEFT));
            nData.add(new MenuElement_HoverElement_Type_Text("" + Game.getCiv(InGame_Alliance.iCivID).getCivName(), CFG.FONT_BOLD_SMALL, Colors.HOVER_GOLD));
        ...
        if (Game.getCiv(InGame_Alliance.iCivID).diplomacy.isRival(Game.player.iCivID)) {
            nData.add(new MenuElement_HoverElement_Type_Text(Game.lang.get("Rival") + ": ", CFG.FONT_REGULAR_SMALL, Colors.HOVER_LEFT));
        ...
        if (DiplomacyManager.getMaxNumberOfAlliances(InGame_Alliance.iCivID) <= Game.getCiv(InGame_Alliance.iCivID).diplomacy.alliance.size() || DiplomacyManager.getMaxNumberOfAlliances(Game.player.iCivID) <= Game.getCiv(Game.player.iCivID).diplomacy.alliance.size()) {
            nData.add(new MenuElement_HoverElement_Type_Text(Game.lang.get("MaxNumOfAlliances") + ": ", CFG.FONT_REGULAR_SMALL, Colors.HOVER_LEFT));
            nData.add(new MenuElement_HoverElement_Type_Text("" + CFG.getPrecision2((float)GameValues.diplomacy.ALLIANCE_SCORE_MAX_NUM_OF_ALLIANCES, 10), CFG.FONT_BOLD_SMALL, Colors.HOVER_GOLD));
            nData.add(new MenuElement_HoverElement_Type_Image(Images.alliance, CFG.PADDING, 0));
            nElements.add(new MenuElement_HoverElement(nData));
            nData.clear();
        }
    ...
    static {
        InGame_Alliance.lTime = 0L;
        InGame_Alliance.iCivID = 0;
    }
}
