...
public class InGame_MessageAlliance extends Menu
{
    ...
    public InGame_MessageAlliance(final String nKey) {
        ...
        InGame_MessageAlliance.key = nKey;
        final PMessage pMessage = Game.player.getMessage(InGame_MessageAlliance.key);
        if (pMessage != null) {
            InGame_MessageAlliance.iCivID = pMessage.fromCivID;
            ...
            menuElements.add(new ButtonFlag_Diplomacy(InGame_MessageAlliance.iCivID, menuWidth / 2 - maxWidth / 2 - CFG.PADDING * 2 - ImageManager.getImage(Images.flagDiplomacyOver).getWidth(), buttonY + tempTitlePaddingY, true));
            ...
            menuElements.add(new Text_Static_ID(InGame_MessageAlliance.iCivID, Game.getCiv(InGame_MessageAlliance.iCivID).getCivName(), CFG.FONT_REGULAR, -1, paddingLeft, buttonY, tempTextW, tempTitleH));
            ...
            menuElements.add(new ButtonStatsRectIMG_Diplomacy(Game.lang.get("Opinion") + ": " + CFG.getPrecision2(Game.getCiv(InGame_MessageAlliance.iCivID).diplomacy.getRelation(Game.player.iCivID), 1), Images.relations, statsX, buttonY, statsW * 2 + CFG.PADDING / 2 * 2, statsH, maxIconW, 0) {
                @Override
                ...
                public void buildElementHover() {
                    this.menuElementHover = InGame_Civ.getHoverBetweenCivilizations(InGame_MessageAlliance.iCivID, Game.player.iCivID, Game.getCiv(Game.player.iCivID).diplomacy.isImprovingRelations(InGame_MessageAlliance.iCivID), Game.getCiv(Game.player.iCivID).diplomacy.isDamagingRelations(InGame_MessageAlliance.iCivID));
                }
            });
            menuElements.add(new ButtonStatsRectIMG_Diplomacy("" + Game.getCiv(InGame_MessageAlliance.iCivID).getNumOfProvinces(), Images.provinces, statsX, buttonY + CFG.PADDING + statsH, statsW, statsH, maxIconW, 0) {
                @Override
                public void buildElementHover() {
            ...
            RulersManager.loadRulerIMG_DiplomacyLeft(InGame_MessageAlliance.iCivID);
            RulersManager.loadRulerIMG_DiplomacyRight(Game.player.iCivID);
            menuElements.add(new ButtonRuler_Diplomacy(InGame_MessageAlliance.iCivID, buttonX + CFG.PADDING, buttonY));
            ...
            menuElements.add(new ButtonGame(Game.lang.get("Refuse"), CFG.FONT_REGULAR, -1, paddingLeft, buttonY, (menuWidth - paddingLeft * 2 - CFG.PADDING / 2 * 2) / 2, true) {
                ...
                public void actionElement() {
                    final PMessage tMessage = Game.player.getMessage(InGame_MessageAlliance.key);
                    if (tMessage != null) {
                        tMessage.onRefuse();
                    }
                    Game.menuManager.setVisibleInGame_PopUp(false);
                    Game.player.removeMessage(InGame_MessageAlliance.key);
                    Game.getCiv(InGame_MessageAlliance.iCivID).diplomacy.setRelation(InGame_MessageAlliance.iCivID, Game.player.iCivID, GameValues.diplomacy.DIPLOMACY_REFUSE_ALLIANCE_OPINION_CHANGE_TO);
                    Game.getCiv(Game.player.iCivID).diplomacy.setRelation(Game.player.iCivID, InGame_MessageAlliance.iCivID, GameValues.diplomacy.DIPLOMACY_REFUSE_ALLIANCE_OPINION_CHANGE_TO);
                }
                ...
                public void buildElementHover() {
                    ...
                    if (tMessage != null) {
                        final List<MenuElement_HoverElement> nElements = new ArrayList<MenuElement_HoverElement>();
                        final List<MenuElement_HoverElement_Type> nData = new ArrayList<MenuElement_HoverElement_Type>();
                        nData.add(new MenuElement_HoverElement_Type_TextTitle_BG(Game.lang.get("Refuse"), CFG.FONT_BOLD, Colors.HOVER_NEGATIVE));
                        nData.add(new MenuElement_HoverElement_Type_ImageTitle_BG(Images.alliance, CFG.PADDING, 0));
                        nElements.add(new MenuElement_HoverElement(nData));
                        nData.clear();
                        nData.add(new MenuElement_HoverElement_Type_Button_TextBonus(Game.lang.get("Relations") + " -> ", CFG.getPrecision2(GameValues.diplomacy.DIPLOMACY_REFUSE_ALLIANCE_OPINION_CHANGE_TO, 10), Images.relationsDown, CFG.FONT_REGULAR_SMALL, CFG.FONT_BOLD_SMALL, Colors.HOVER_LEFT, Colors.HOVER_GOLD));
                        nElements.add(new MenuElement_HoverElement(nData));
                        nData.clear();
                        this.menuElementHover = new MenuElement_Hover(nElements);
                    }
            ...
            menuElements.add(new ButtonGame(Game.lang.get("Accept"), CFG.FONT_REGULAR, -1, paddingLeft + CFG.PADDING + (menuWidth - paddingLeft * 2 - CFG.PADDING / 2 * 2) / 2, buttonY, (menuWidth - paddingLeft * 2 - CFG.PADDING / 2 * 2) / 2, true) {
                ...
                public void buildElementHover() {
                    ...
                    if (tMessage != null) {
                        final List<MenuElement_HoverElement> nElements = new ArrayList<MenuElement_HoverElement>();
                        final List<MenuElement_HoverElement_Type> nData = new ArrayList<MenuElement_HoverElement_Type>();
                        nData.add(new MenuElement_HoverElement_Type_TextTitle_BG(Game.lang.get("FormAlliance"), CFG.FONT_BOLD, Colors.HOVER_GOLD));
                        nData.add(new MenuElement_HoverElement_Type_ImageTitle_BG(Images.alliance, CFG.PADDING, 0));
                        nElements.add(new MenuElement_HoverElement(nData));
                        nData.clear();
        ...
        this.initMenu(new MenuTitleIMG_Message(Game.lang.get("FormAlliance"), true, false, Images.title600) {
            @Override
            public long getTime() {
                return InGame_MessageAlliance.lTime;
            }
        ...
        InGame_MessageAlliance.lTime = CFG.currentTimeMillis;
    }
    ...
    public void draw(final SpriteBatch oSB, final int iTranslateX, int iTranslateY, final boolean menuIsActive, final Status titleStatus) {
        ...
        if (InGame_MessageAlliance.lTime + 60L >= CFG.currentTimeMillis) {
            iTranslateY = iTranslateY - CFG.BUTTON_HEIGHT * 3 / 5 + (int)(CFG.BUTTON_HEIGHT * 3 / 5 * ((CFG.currentTimeMillis - InGame_MessageAlliance.lTime) / 60.0f));
        }
    ...
    static {
        InGame_MessageAlliance.lTime = 0L;
        InGame_MessageAlliance.iCivID = 0;
    }
}
