...
public class InGame_Civ_Government extends Menu
{
    protected static final int ANIMATION_TIME = 60;
    public static long lTime;
    public static long lTime2;
    public static int iSortID;
    public static int iGovID;
    
    public InGame_Civ_Government() {
        ...
        menuElements.add(new ButtonIdeology2(InGame_Civ_Government.iGovID, buttonX, buttonY, menuWidth - buttonX * 2, religionH) {
            @Override
        ...
        final Graph_Vertical graphVertical = new Graph_Vertical(Graph_Vertical_Data_Type.GOVERNMENTS_CIVS, Game.lang.get("Civilizations"), Game.lang.get("Civilizations"), paddingLeft, buttonY, menuWidth - paddingLeft * 2, CFG.BUTTON_HEIGHT * 3, true) {};
        menuElements.add(graphVertical);
        buttonY += menuElements.get(menuElements.size() - 1).getHeight() + CFG.PADDING;
        buttonX = Images.boxTitleBORDERWIDTH;
        menuElements.add(new Text_TitleBlueSort(InGame_Civ_Government.iSortID == 0 || InGame_Civ_Government.iSortID == 1, InGame_Civ_Government.iSortID == 1, Game.lang.get("Name"), -1, buttonX, buttonY, p0W, CFG.TEXT_HEIGHT + CFG.PADDING * 6, CFG.FONT_BOLD_SMALL) {
            @Override
            ...
            @Override
            public void actionElement() {
                if (InGame_Civ_Government.iSortID == 0) {
                    InGame_Civ_Government.iSortID = 1;
                }
                else {
                    InGame_Civ_Government.iSortID = 0;
                }
                Game.menuManager.rebuildInGame_Civ_Government();
                InGame_Civ_Government.lTime = 0L;
            }
        ...
        menuElements.add(new Text_TitleBlueSort(InGame_Civ_Government.iSortID == 2 || InGame_Civ_Government.iSortID == 3, InGame_Civ_Government.iSortID == 3, Game.lang.get("Population"), -1, buttonX, buttonY, p1W, CFG.TEXT_HEIGHT + CFG.PADDING * 6, CFG.FONT_BOLD_SMALL) {
            @Override
            public void drawLines(final SpriteBatch oSB, final int iTranslateX, final int iTranslateY, final boolean isActive, final boolean scrollableY) {
            }
            ...
            @Override
            public void actionElement() {
                if (InGame_Civ_Government.iSortID == 2) {
                    InGame_Civ_Government.iSortID = 3;
                }
                else {
                    InGame_Civ_Government.iSortID = 2;
                }
                Game.menuManager.rebuildInGame_Civ_Government();
                InGame_Civ_Government.lTime = 0L;
            }
        ...
        for (int i = 0; i < Game.getCivsSize(); ++i) {
            tCivsID.add(i);
            if (Game.getCiv(i).getIdeologyID() == InGame_Civ_Government.iGovID) {
        ...
        if (tCivsID.size() > 0) {
            while (tCivsID.size() > 0) {
                ...
                else if (InGame_Civ_Government.iSortID == 1) {
                    for (int o = 1; o < tCivsID.size(); ++o) {
                ...
                else if (InGame_Civ_Government.iSortID == 2) {
                    for (int o = 1; o < tCivsID.size(); ++o) {
                ...
                else if (InGame_Civ_Government.iSortID == 3) {
                    for (int o = 1; o < tCivsID.size(); ++o) {
                        if (tPop.get(toAddID) > tPop.get(o)) {
                            toAddID = o;
                        }
                ...
                menuElements.add(new Text_StaticBG_ID_FlagCiv(Game.getCiv(tCivsID.get(toAddID)).getCivName(), CFG.FONT_REGULAR_SMALL, CFG.PADDING * 2, buttonX, buttonY, p0W2, buttonH, (int)tCivsID.get(toAddID)) {
                    ...
                    public void buildElementHover() {
                        final List<MenuElement_HoverElement> nElements = new ArrayList<MenuElement_HoverElement>();
                        final List<MenuElement_HoverElement_Type> nData = new ArrayList<MenuElement_HoverElement_Type>();
                        nData.add(new MenuElement_HoverElement_Type_FlagCiv_Title(this.getCurrent(), Game.ideologiesManager.getIdeology(Game.getCiv(this.getCurrent()).getIdeologyID()).Name));
                        nElements.add(new MenuElement_HoverElement(nData));
                        nData.clear();
        ...
        this.initMenu(new MenuTitleIMG_FlagCenter(Game.getCiv(InGame_Civ.iActiveCivID).getCivName(), Game.lang.get("Government"), false, false, Images.title500) {
            @Override
            ...
            @Override
            public long getTime() {
                return InGame_Civ_Government.lTime2;
            }
    ...
    public void draw(final SpriteBatch oSB, int iTranslateX, final int iTranslateY, final boolean menuIsActive, final Status titleStatus) {
        if (InGame_Civ_Government.lTime + 60L >= CFG.currentTimeMillis) {
            iTranslateX = iTranslateX - CFG.BUTTON_WIDTH + (int)(CFG.BUTTON_WIDTH * ((CFG.currentTimeMillis - InGame_Civ_Government.lTime) / 60.0f));
        }
    ...
    public void setVisible(final boolean visible) {
        ...
        InGame_Civ_Government.lTime = CFG.currentTimeMillis;
        InGame_Civ_Government.lTime2 = InGame_Civ_Government.lTime;
    }
    ...
    static {
        InGame_Civ_Government.lTime = 0L;
        InGame_Civ_Government.lTime2 = 0L;
        InGame_Civ_Government.iSortID = 2;
        InGame_Civ_Government.iGovID = -1;
    }
}
