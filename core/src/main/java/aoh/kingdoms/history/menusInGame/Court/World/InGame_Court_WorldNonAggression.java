...
public class InGame_Court_WorldNonAggression extends Menu
{
    public InGame_Court_WorldNonAggression() {
        ...
        final int maxWidth = ImageManager.getImage(Images.nonAggression).getWidth() + CFG.PADDING * 4;
        ...
        menuElements.add(new Text_Title_v2Center(Game.lang.get("NonAggressionPacts"), -1, Images.boxTitleBORDERWIDTH, buttonY, menuWidth - Images.boxTitleBORDERWIDTH * 2, CFG.TEXT_HEIGHT + CFG.PADDING * 6));
        buttonY += menuElements.get(menuElements.size() - 1).getHeight() + CFG.PADDING;
        final int tElements = menuElements.size();
        for (int i = 1; i < Game.getCivsSize(); ++i) {
            if (Game.getCiv(i).diplomacy.nonAggressionPact.size() > 0) {
                for (final Map.Entry<Integer, Diplomacy.DiplomacyData> entry : Game.getCiv(i).diplomacy.nonAggressionPact.entrySet()) {
                    if (entry.getKey() > i) {
                        ...
                        menuElements.add(new TextIcon_Diplomacy(Images.nonAggression, paddingLeft, buttonY, menuWidth - paddingLeft * 2, tempTitleH, maxWidth) {
                            @Override
