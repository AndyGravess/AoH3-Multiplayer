...
public class InGame_MessageNonAggressionPact extends Menu
{
    ...
    public InGame_MessageNonAggressionPact(final String nKey) {
        ...
        InGame_MessageNonAggressionPact.key = nKey;
        final PMessage pMessage = Game.player.getMessage(InGame_MessageNonAggressionPact.key);
        if (pMessage != null) {
            InGame_MessageNonAggressionPact.iCivID = pMessage.fromCivID;
            ...
            menuElements.add(new ButtonFlag_Diplomacy(InGame_MessageNonAggressionPact.iCivID, menuWidth / 2 - maxWidth / 2 - CFG.PADDING * 2 - ImageManager.getImage(Images.flagDiplomacyOver).getWidth(), buttonY + tempTitlePaddingY, true));
            ...
            menuElements.add(new Text_Static_ID(InGame_MessageNonAggressionPact.iCivID, Game.getCiv(InGame_MessageNonAggressionPact.iCivID).getCivName(), CFG.FONT_REGULAR, -1, paddingLeft, buttonY, tempTextW, tempTitleH));
            ...
            menuElements.add(new ButtonStatsRectIMG_Diplomacy(Game.lang.get("Opinion") + ": " + CFG.getPrecision2(Game.getCiv(InGame_MessageNonAggressionPact.iCivID).diplomacy.getRelation(Game.player.iCivID), 1), Images.relations, statsX, buttonY, statsW * 2 + CFG.PADDING / 2 * 2, statsH, maxIconW, 0) {
                @Override
                ...
                public void buildElementHover() {
                    this.menuElementHover = InGame_Civ.getHoverBetweenCivilizations(InGame_MessageNonAggressionPact.iCivID, Game.player.iCivID, Game.getCiv(Game.player.iCivID).diplomacy.isImprovingRelations(InGame_MessageNonAggressionPact.iCivID), Game.getCiv(Game.player.iCivID).diplomacy.isDamagingRelations(InGame_MessageNonAggressionPact.iCivID));
                }
            });
            menuElements.add(new ButtonStatsRectIMG_Diplomacy("" + Game.getCiv(InGame_MessageNonAggressionPact.iCivID).getNumOfProvinces(), Images.provinces, statsX, buttonY + CFG.PADDING + statsH, statsW, statsH, maxIconW, 0) {
                @Override
                public void buildElementHover() {
            ...
            menuElements.add(new ButtonStatsRectIMG_Diplomacy(Game_Calendar.getDate_ByTurnID(Game_Calendar.TURN_ID + GameValues.diplomacy.DIPLOMACY_NON_AGGRESSION_PACT_EXPIRES), Images.time, statsX, buttonY + (CFG.PADDING + statsH) * 2, statsW * 2 + CFG.PADDING, statsH, maxIconW, 0) {
                @Override
                public void buildElementHover() {
                    final List<MenuElement_HoverElement> nElements = new ArrayList<MenuElement_HoverElement>();
                    final List<MenuElement_HoverElement_Type> nData = new ArrayList<MenuElement_HoverElement_Type>();
            ...
            RulersManager.loadRulerIMG_DiplomacyLeft(InGame_MessageNonAggressionPact.iCivID);
            RulersManager.loadRulerIMG_DiplomacyRight(Game.player.iCivID);
            menuElements.add(new ButtonRuler_Diplomacy(InGame_MessageNonAggressionPact.iCivID, buttonX + CFG.PADDING, buttonY));
            ...
            menuElements.add(new ButtonGame(Game.lang.get("Refuse"), CFG.FONT_REGULAR, -1, paddingLeft, buttonY, (menuWidth - paddingLeft * 2 - CFG.PADDING / 2 * 2) / 2, true) {
                ...
                public void actionElement() {
                    final PMessage tMessage = Game.player.getMessage(InGame_MessageNonAggressionPact.key);
                    if (tMessage != null) {
                        tMessage.onRefuse();
                    }
                    Game.menuManager.setVisibleInGame_PopUp(false);
                    Game.player.removeMessage(InGame_MessageNonAggressionPact.key);
                }
            });
            menuElements.add(new ButtonGame(Game.lang.get("Accept"), CFG.FONT_REGULAR, -1, paddingLeft + CFG.PADDING + (menuWidth - paddingLeft * 2 - CFG.PADDING / 2 * 2) / 2, buttonY, (menuWidth - paddingLeft * 2 - CFG.PADDING / 2 * 2) / 2, true) {
                ...
                public void buildElementHover() {
                    ...
                    if (tMessage != null) {
                        final List<MenuElement_HoverElement> nElements = new ArrayList<MenuElement_HoverElement>();
                        final List<MenuElement_HoverElement_Type> nData = new ArrayList<MenuElement_HoverElement_Type>();
                        nData.add(new MenuElement_HoverElement_Type_TextTitle_BG(Game.lang.get("SignNonAggressionPactWithCivA", Game.getCiv(tMessage.fromCivID).getCivName()), CFG.FONT_BOLD, Colors.HOVER_GOLD));
        ...
        int withCivID = 0;
        if (pMessage != null) {
            withCivID = pMessage.fromCivID;
        }
        this.initMenu(new MenuTitleIMG_Message(Game.lang.get("SignNonAggressionPactWithCivA", Game.getCiv(withCivID).getCivName()), true, false, Images.title600) {
            @Override
            public long getTime() {
                return InGame_MessageNonAggressionPact.lTime;
            }
        ...
        InGame_MessageNonAggressionPact.lTime = CFG.currentTimeMillis;
    }
    ...
    public void draw(final SpriteBatch oSB, final int iTranslateX, int iTranslateY, final boolean menuIsActive, final Status titleStatus) {
        ...
        if (InGame_MessageNonAggressionPact.lTime + 60L >= CFG.currentTimeMillis) {
            iTranslateY = iTranslateY - CFG.BUTTON_HEIGHT * 3 / 5 + (int)(CFG.BUTTON_HEIGHT * 3 / 5 * ((CFG.currentTimeMillis - InGame_MessageNonAggressionPact.lTime) / 60.0f));
        }
    ...
    static {
        InGame_MessageNonAggressionPact.lTime = 0L;
        InGame_MessageNonAggressionPact.iCivID = 0;
    }
}
