...
public class InGame_MessageDefensivePact extends Menu
{
    ...
    public InGame_MessageDefensivePact(final String nKey) {
        ...
        InGame_MessageDefensivePact.key = nKey;
        final PMessage pMessage = Game.player.getMessage(InGame_MessageDefensivePact.key);
        if (pMessage != null) {
            InGame_MessageDefensivePact.iCivID = pMessage.fromCivID;
            ...
            menuElements.add(new ButtonFlag_Diplomacy(InGame_MessageDefensivePact.iCivID, menuWidth / 2 - maxWidth / 2 - CFG.PADDING * 2 - ImageManager.getImage(Images.flagDiplomacyOver).getWidth(), buttonY + tempTitlePaddingY, true));
            ...
            menuElements.add(new Text_Static_ID(InGame_MessageDefensivePact.iCivID, Game.getCiv(InGame_MessageDefensivePact.iCivID).getCivName(), CFG.FONT_REGULAR, -1, paddingLeft, buttonY, tempTextW, tempTitleH));
            ...
            menuElements.add(new ButtonStatsRectIMG_Diplomacy(Game.lang.get("Opinion") + ": " + CFG.getPrecision2(Game.getCiv(InGame_MessageDefensivePact.iCivID).diplomacy.getRelation(Game.player.iCivID), 1), Images.relations, statsX, buttonY, statsW * 2 + CFG.PADDING / 2 * 2, statsH, maxIconW, 0) {
                @Override
                ...
                public void buildElementHover() {
                    this.menuElementHover = InGame_Civ.getHoverBetweenCivilizations(InGame_MessageDefensivePact.iCivID, Game.player.iCivID, Game.getCiv(Game.player.iCivID).diplomacy.isImprovingRelations(InGame_MessageDefensivePact.iCivID), Game.getCiv(Game.player.iCivID).diplomacy.isDamagingRelations(InGame_MessageDefensivePact.iCivID));
                }
            });
            menuElements.add(new ButtonStatsRectIMG_Diplomacy("" + Game.getCiv(InGame_MessageDefensivePact.iCivID).getNumOfProvinces(), Images.provinces, statsX, buttonY + CFG.PADDING + statsH, statsW, statsH, maxIconW, 0) {
                @Override
                public void buildElementHover() {
            ...
            menuElements.add(new ButtonStatsRectIMG_Diplomacy(Game_Calendar.getDate_ByTurnID(Game_Calendar.TURN_ID + GameValues.diplomacy.DIPLOMACY_DEFENSIVE_PACT_EXPIRES), Images.time, statsX, buttonY + (CFG.PADDING + statsH) * 2, statsW * 2 + CFG.PADDING, statsH, maxIconW, 0) {
                @Override
                public void buildElementHover() {
                    final List<MenuElement_HoverElement> nElements = new ArrayList<MenuElement_HoverElement>();
                    final List<MenuElement_HoverElement_Type> nData = new ArrayList<MenuElement_HoverElement_Type>();
            ...
            RulersManager.loadRulerIMG_DiplomacyLeft(InGame_MessageDefensivePact.iCivID);
            RulersManager.loadRulerIMG_DiplomacyRight(Game.player.iCivID);
            menuElements.add(new ButtonRuler_Diplomacy(InGame_MessageDefensivePact.iCivID, buttonX + CFG.PADDING, buttonY));
            ...
            menuElements.add(new ButtonGame(Game.lang.get("Refuse"), CFG.FONT_REGULAR, -1, paddingLeft, buttonY, (menuWidth - paddingLeft * 2 - CFG.PADDING / 2 * 2) / 2, true) {
                ...
                public void actionElement() {
                    final PMessage tMessage = Game.player.getMessage(InGame_MessageDefensivePact.key);
                    if (tMessage != null) {
                        tMessage.onRefuse();
                    }
                    Game.menuManager.setVisibleInGame_PopUp(false);
                    Game.player.removeMessage(InGame_MessageDefensivePact.key);
                }
            });
            menuElements.add(new ButtonGame(Game.lang.get("Accept"), CFG.FONT_REGULAR, -1, paddingLeft + CFG.PADDING + (menuWidth - paddingLeft * 2 - CFG.PADDING / 2 * 2) / 2, buttonY, (menuWidth - paddingLeft * 2 - CFG.PADDING / 2 * 2) / 2, true) {
                ...
                public void buildElementHover() {
                    ...
                    if (tMessage != null) {
                        final List<MenuElement_HoverElement> nElements = new ArrayList<MenuElement_HoverElement>();
                        final List<MenuElement_HoverElement_Type> nData = new ArrayList<MenuElement_HoverElement_Type>();
                        nData.add(new MenuElement_HoverElement_Type_TextTitle_BG(Game.lang.get("SignDefensivePactWithCivA", Game.getCiv(tMessage.fromCivID).getCivName()), CFG.FONT_BOLD, Colors.HOVER_GOLD));
        ...
        this.initMenu(new MenuTitleIMG_Message(Game.lang.get("SignDefensivePactWithCivA", Game.getCiv(withCivID).getCivName()), true, false, Images.title600) {
            @Override
            public long getTime() {
                return InGame_MessageDefensivePact.lTime;
            }
        ...
        InGame_MessageDefensivePact.lTime = CFG.currentTimeMillis;
    }
    ...
    public void draw(final SpriteBatch oSB, final int iTranslateX, int iTranslateY, final boolean menuIsActive, final Status titleStatus) {
        ...
        if (InGame_MessageDefensivePact.lTime + 60L >= CFG.currentTimeMillis) {
            iTranslateY = iTranslateY - CFG.BUTTON_HEIGHT * 3 / 5 + (int)(CFG.BUTTON_HEIGHT * 3 / 5 * ((CFG.currentTimeMillis - InGame_MessageDefensivePact.lTime) / 60.0f));
        }
    ...
    static {
        InGame_MessageDefensivePact.lTime = 0L;
        InGame_MessageDefensivePact.iCivID = 0;
    }
}
