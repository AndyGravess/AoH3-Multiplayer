...
public class InGame_MessageDemandsMilitaryAccess extends Menu
{
    ...
    public InGame_MessageDemandsMilitaryAccess(final String nKey) {
        ...
        InGame_MessageDemandsMilitaryAccess.key = nKey;
        final PMessage pMessage = Game.player.getMessage(InGame_MessageDemandsMilitaryAccess.key);
        if (pMessage != null) {
            InGame_MessageDemandsMilitaryAccess.iCivID = pMessage.fromCivID;
            ...
            menuElements.add(new ButtonFlag_Diplomacy(InGame_MessageDemandsMilitaryAccess.iCivID, menuWidth / 2 - maxWidth / 2 - CFG.PADDING * 2 - ImageManager.getImage(Images.flagDiplomacyOver).getWidth(), buttonY + tempTitlePaddingY, true));
            ...
            menuElements.add(new Text_Static_ID(InGame_MessageDemandsMilitaryAccess.iCivID, Game.getCiv(InGame_MessageDemandsMilitaryAccess.iCivID).getCivName(), CFG.FONT_REGULAR, -1, paddingLeft, buttonY, tempTextW, tempTitleH));
            ...
            menuElements.add(new ButtonStatsRectIMG_Diplomacy(Game.lang.get("Opinion") + ": " + CFG.getPrecision2(Game.getCiv(InGame_MessageDemandsMilitaryAccess.iCivID).diplomacy.getRelation(Game.player.iCivID), 1), Images.relations, statsX, buttonY, statsW * 2 + CFG.PADDING / 2 * 2, statsH, maxIconW, 0) {
                @Override
                ...
                public void buildElementHover() {
                    this.menuElementHover = InGame_Civ.getHoverBetweenCivilizations(InGame_MessageDemandsMilitaryAccess.iCivID, Game.player.iCivID, Game.getCiv(Game.player.iCivID).diplomacy.isImprovingRelations(InGame_MessageDemandsMilitaryAccess.iCivID), Game.getCiv(Game.player.iCivID).diplomacy.isDamagingRelations(InGame_MessageDemandsMilitaryAccess.iCivID));
                }
            });
            menuElements.add(new ButtonStatsRectIMG_Diplomacy("" + Game.getCiv(InGame_MessageDemandsMilitaryAccess.iCivID).getNumOfProvinces(), Images.provinces, statsX, buttonY + CFG.PADDING + statsH, statsW, statsH, maxIconW, 0) {
                @Override
                public void buildElementHover() {
            ...
            menuElements.add(new ButtonStatsRectIMG_Diplomacy(Game_Calendar.getDate_ByTurnID(Game_Calendar.TURN_ID + GameValues.diplomacy.DIPLOMACY_MILITARY_ACCESS_EXPIRES), Images.time, statsX, buttonY + (CFG.PADDING + statsH) * 2, statsW * 2 + CFG.PADDING, statsH, maxIconW, 0) {
                @Override
                public void buildElementHover() {
                    final List<MenuElement_HoverElement> nElements = new ArrayList<MenuElement_HoverElement>();
                    final List<MenuElement_HoverElement_Type> nData = new ArrayList<MenuElement_HoverElement_Type>();
            ...
            RulersManager.loadRulerIMG_DiplomacyLeft(InGame_MessageDemandsMilitaryAccess.iCivID);
            RulersManager.loadRulerIMG_DiplomacyRight(Game.player.iCivID);
            menuElements.add(new ButtonRuler_Diplomacy(InGame_MessageDemandsMilitaryAccess.iCivID, buttonX + CFG.PADDING, buttonY));
            ...
            menuElements.add(new ButtonGame(Game.lang.get("Refuse"), CFG.FONT_REGULAR, -1, paddingLeft, buttonY, (menuWidth - paddingLeft * 2 - CFG.PADDING / 2 * 2) / 2, true) {
                ...
                public void actionElement() {
                    final PMessage tMessage = Game.player.getMessage(InGame_MessageDemandsMilitaryAccess.key);
                    if (tMessage != null) {
                        tMessage.onRefuse();
                    }
                    Game.menuManager.setVisibleInGame_PopUp(false);
                    Game.player.removeMessage(InGame_MessageDemandsMilitaryAccess.key);
                }
            });
            menuElements.add(new ButtonGame(Game.lang.get("Accept"), CFG.FONT_REGULAR, -1, paddingLeft + CFG.PADDING + (menuWidth - paddingLeft * 2 - CFG.PADDING / 2 * 2) / 2, buttonY, (menuWidth - paddingLeft * 2 - CFG.PADDING / 2 * 2) / 2, true) {
                ...
                public void buildElementHover() {
                    ...
                    if (tMessage != null) {
                        final List<MenuElement_HoverElement> nElements = new ArrayList<MenuElement_HoverElement>();
                        final List<MenuElement_HoverElement_Type> nData = new ArrayList<MenuElement_HoverElement_Type>();
                        nData.add(new MenuElement_HoverElement_Type_TextTitle_BG(Game.lang.get("OfferMilitaryAccess", Game.getCiv(tMessage.fromCivID).getCivName()), CFG.FONT_BOLD, Colors.HOVER_GOLD));
        ...
        this.initMenu(new MenuTitleIMG_Message(Game.lang.get("DemandMilitaryAccess"), true, false, Images.title600) {
            @Override
            public long getTime() {
                return InGame_MessageDemandsMilitaryAccess.lTime;
            }
        ...
        InGame_MessageDemandsMilitaryAccess.lTime = CFG.currentTimeMillis;
    }
    ...
    public void draw(final SpriteBatch oSB, final int iTranslateX, int iTranslateY, final boolean menuIsActive, final Status titleStatus) {
        ...
        if (InGame_MessageDemandsMilitaryAccess.lTime + 60L >= CFG.currentTimeMillis) {
            iTranslateY = iTranslateY - CFG.BUTTON_HEIGHT * 3 / 5 + (int)(CFG.BUTTON_HEIGHT * 3 / 5 * ((CFG.currentTimeMillis - InGame_MessageDemandsMilitaryAccess.lTime) / 60.0f));
        }
    ...
    static {
        InGame_MessageDemandsMilitaryAccess.lTime = 0L;
        InGame_MessageDemandsMilitaryAccess.iCivID = 0;
    }
}
