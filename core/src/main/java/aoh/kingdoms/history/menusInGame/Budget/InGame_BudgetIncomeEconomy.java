...
public class InGame_BudgetIncomeEconomy extends Menu
{
    ...
    public InGame_BudgetIncomeEconomy() {
        ...
        menuElements.add(new ButtonStatsBudget_Right2("+" + CFG.getPrecision2(Game.getCiv(Game.player.iCivID).fTotalIncomePerMonth + Game.getCiv(this.iActiveCivID).civBonuses.MonthlyIncome, 100), paddingLeft + tButtonW + CFG.PADDING, buttonY, tButtonRightW, tButtonH) {
            ...
            public String getTextToDraw() {
                if (this.lastValue != Game.getCiv(Game.player.iCivID).fTotalIncomePerMonth + Game.getCiv(InGame_BudgetIncomeEconomy.this.iActiveCivID).civBonuses.MonthlyIncome) {
                    final float fVal = Game.getCiv(Game.player.iCivID).fTotalIncomePerMonth + Game.getCiv(InGame_BudgetIncomeEconomy.this.iActiveCivID).civBonuses.MonthlyIncome;
        ...
        menuElements.add(new Text_TitleBlueSort(InGame_BudgetIncomeEconomy.iSortID == 0 || InGame_BudgetIncomeEconomy.iSortID == 1, InGame_BudgetIncomeEconomy.iSortID == 1, Game.lang.get("Name"), -1, buttonX, buttonY, r0W, CFG.TEXT_HEIGHT + CFG.PADDING * 6, CFG.FONT_BOLD_SMALL) {
            @Override
            ...
            @Override
            public void actionElement() {
                if (InGame_BudgetIncomeEconomy.iSortID == 0) {
                    InGame_BudgetIncomeEconomy.iSortID = 1;
                }
                else {
                    InGame_BudgetIncomeEconomy.iSortID = 0;
                }
                Game.menuManager.rebuildInGame_BudgetIncomeEconomy();
                InGame_BudgetIncomeEconomy.lTime = 0L;
            }
        ...
        menuElements.add(new Text_TitleBlueSort(InGame_BudgetIncomeEconomy.iSortID == 2 || InGame_BudgetIncomeEconomy.iSortID == 3, InGame_BudgetIncomeEconomy.iSortID == 3, Game.lang.get("Population"), -1, buttonX, buttonY, r0W, CFG.TEXT_HEIGHT + CFG.PADDING * 6, CFG.FONT_BOLD_SMALL) {
            @Override
            public void drawLines(final SpriteBatch oSB, final int iTranslateX, final int iTranslateY, final boolean isActive, final boolean scrollableY) {
            }
            ...
            @Override
            public void actionElement() {
                if (InGame_BudgetIncomeEconomy.iSortID == 2) {
                    InGame_BudgetIncomeEconomy.iSortID = 3;
                }
                else {
                    InGame_BudgetIncomeEconomy.iSortID = 2;
                }
                Game.menuManager.rebuildInGame_BudgetIncomeEconomy();
                InGame_BudgetIncomeEconomy.lTime = 0L;
            }
        ...
        buttonX += menuElements.get(menuElements.size() - 1).getWidth();
        menuElements.add(new Text_TitleBlueSort(InGame_BudgetIncomeEconomy.iSortID == 4 || InGame_BudgetIncomeEconomy.iSortID == 5, InGame_BudgetIncomeEconomy.iSortID == 5, Game.lang.get("Economy"), -1, buttonX, buttonY, r1W, CFG.TEXT_HEIGHT + CFG.PADDING * 6, CFG.FONT_BOLD_SMALL) {
            @Override
            public void drawLines(final SpriteBatch oSB, final int iTranslateX, final int iTranslateY, final boolean isActive, final boolean scrollableY) {
            }
            ...
            @Override
            public void actionElement() {
                if (InGame_BudgetIncomeEconomy.iSortID == 4) {
                    InGame_BudgetIncomeEconomy.iSortID = 5;
                }
                else {
                    InGame_BudgetIncomeEconomy.iSortID = 4;
                }
                Game.menuManager.rebuildInGame_BudgetIncomeEconomy();
                InGame_BudgetIncomeEconomy.lTime = 0L;
            }
        ...
        buttonX += menuElements.get(menuElements.size() - 1).getWidth();
        menuElements.add(new Text_TitleBlueSort(InGame_BudgetIncomeEconomy.iSortID == 6 || InGame_BudgetIncomeEconomy.iSortID == 7, InGame_BudgetIncomeEconomy.iSortID == 7, Game.lang.get("Income"), -1, buttonX, buttonY, r1W, CFG.TEXT_HEIGHT + CFG.PADDING * 6, CFG.FONT_BOLD_SMALL) {
            @Override
            public void drawLines(final SpriteBatch oSB, final int iTranslateX, final int iTranslateY, final boolean isActive, final boolean scrollableY) {
            }
            ...
            @Override
            public void actionElement() {
                if (InGame_BudgetIncomeEconomy.iSortID == 6) {
                    InGame_BudgetIncomeEconomy.iSortID = 7;
                }
                else {
                    InGame_BudgetIncomeEconomy.iSortID = 6;
                }
                Game.menuManager.rebuildInGame_BudgetIncomeEconomy();
                InGame_BudgetIncomeEconomy.lTime = 0L;
            }
        ...
        while (tProvinces.size() > 0) {
            ...
            else if (InGame_BudgetIncomeEconomy.iSortID == 1) {
                for (int o = 1; o < tProvinces.size(); ++o) {
            ...
            else if (InGame_BudgetIncomeEconomy.iSortID == 2) {
                for (int o = 1; o < tProvinces.size(); ++o) {
            ...
            else if (InGame_BudgetIncomeEconomy.iSortID == 3) {
                for (int o = 1; o < tProvinces.size(); ++o) {
                    if (Game.getProvince(tProvinces.get(o)).getPopulationTotal() < Game.getProvince(tProvinces.get(toAddID)).getPopulationTotal()) {
                        toAddID = o;
                    }
            ...
            else if (InGame_BudgetIncomeEconomy.iSortID == 4) {
                for (int o = 1; o < tProvinces.size(); ++o) {
            ...
            else if (InGame_BudgetIncomeEconomy.iSortID == 5) {
                for (int o = 1; o < tProvinces.size(); ++o) {
                    if (Game.getProvince(tProvinces.get(o)).getEconomyWithBonuses() < Game.getProvince(tProvinces.get(toAddID)).getEconomyWithBonuses()) {
                        toAddID = o;
                    }
            ...
            else if (InGame_BudgetIncomeEconomy.iSortID == 6) {
                for (int o = 1; o < tProvinces.size(); ++o) {
            ...
            else if (InGame_BudgetIncomeEconomy.iSortID == 7) {
                for (int o = 1; o < tProvinces.size(); ++o) {
                    if (Game.getIncomeFromEconomy(tProvinces.get(toAddID)) > Game.getIncomeFromEconomy(tProvinces.get(o))) {
                        toAddID = o;
                    }
            ...
            menuElements.add(new ButtonStatsBudget_Right2(((fGold > 0.0f) ? "+" : "") + CFG.getPrecision2(fGold, 100), buttonX, buttonY, r1W, buttonH, (int)tProvinces.get(toAddID)) {
                @Override
                public String getTextToDraw() {
                    if (this.lastValue != Game.getIncomeFromEconomy(this.id)) {
                        final float fVal = Game.getIncomeFromEconomy(this.id);
                        this.setText(((fVal > 0.0f) ? "+" : "") + CFG.getPrecision2(fVal, 100));
                        this.lastValue = fVal;
                        if (fVal == 0.0f) {
                            this.nColor = Colors.COLOR_TEXT_MODIFIER_NEUTRAL;
                ...
                public void buildElementHover() {
                    ...
                    nData.add(new MenuElement_HoverElement_Type_TextTitle(Game.lang.get("MonthlyIncomeEconomy") + ": ", CFG.FONT_BOLD));
        ...
        this.initMenu(new MenuTitleIMG_FlagCenter(Game.lang.get("MonthlyIncomeEconomy"), Game.getCiv(this.iActiveCivID).getCivName(), false, false, Images.title500) {
            ...
            public int getFlagCivID() {
                return InGame_BudgetIncomeEconomy.this.iActiveCivID;
            }
            
            @Override
            public long getTime() {
                return InGame_BudgetIncomeEconomy.lTime2;
            }
    ...
    public void draw(final SpriteBatch oSB, int iTranslateX, final int iTranslateY, final boolean menuIsActive, final Status titleStatus) {
        if (InGame_BudgetIncomeEconomy.lTime + 60L >= CFG.currentTimeMillis) {
            iTranslateX = iTranslateX - CFG.BUTTON_WIDTH + (int)(CFG.BUTTON_WIDTH * ((CFG.currentTimeMillis - InGame_BudgetIncomeEconomy.lTime) / 60.0f));
        }
    ...
    public void updateLanguage() {
        ...
        this.getTitle().setText(Game.lang.get("MonthlyIncomeEconomy"));
    }
    
    @Override
    public void setVisible(final boolean visible) {
        super.setVisible(visible);
        InGame_BudgetIncomeEconomy.lTime2 = CFG.currentTimeMillis;
    }
    
    static {
        InGame_BudgetIncomeEconomy.lTime = 0L;
        InGame_BudgetIncomeEconomy.lTime2 = 0L;
        InGame_BudgetIncomeEconomy.iSortID = 6;
    }
}
