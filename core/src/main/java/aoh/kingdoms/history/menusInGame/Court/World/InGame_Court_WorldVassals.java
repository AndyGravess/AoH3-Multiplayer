...
public class InGame_Court_WorldVassals extends Menu
{
    public InGame_Court_WorldVassals() {
        ...
        final int maxWidth = ImageManager.getImage(Images.vassal).getWidth() + CFG.PADDING * 4;
        ...
        menuElements.add(new Text_Title_v2Center(Game.lang.get(Game_Ages.getVassals()), -1, Images.boxTitleBORDERWIDTH, buttonY, menuWidth - Images.boxTitleBORDERWIDTH * 2, CFG.TEXT_HEIGHT + CFG.PADDING * 6));
        buttonY += menuElements.get(menuElements.size() - 1).getHeight() + CFG.PADDING;
        final int tElements = menuElements.size();
        for (int i = 1; i < Game.getCivsSize(); ++i) {
            if (Game.getCiv(i).diplomacy.iVassalsSize > 0) {
                for (int j = 0; j < Game.getCiv(i).diplomacy.iVassalsSize; ++j) {
                    ...
                    menuElements.add(new ButtonFlag_Diplomacy(Game.getCiv(i).diplomacy.lVassals.get(j).c, menuWidth / 2 + maxWidth / 2 + CFG.PADDING * 2, buttonY + tempTitlePaddingY, true));
                    menuElements.add(new Text_Static_ID(Game.getCiv(i).diplomacy.lVassals.get(j).c, Game.getCiv(Game.getCiv(i).diplomacy.lVassals.get(j).c).getCivName(), CFG.FONT_REGULAR_SMALL, -1, menuWidth / 2 + maxWidth / 2 + CFG.PADDING * 2 + ImageManager.getImage(Images.flagDiplomacyOver).getWidth(), buttonY, tempTextW, tempTitleH));
                    ...
                    menuElements.add(new TextIcon_Diplomacy(Images.vassal, paddingLeft, buttonY, menuWidth - paddingLeft * 2, tempTitleH, maxWidth) {
                        @Override
