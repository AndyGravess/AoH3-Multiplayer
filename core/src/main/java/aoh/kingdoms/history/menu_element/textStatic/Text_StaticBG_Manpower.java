...
public class Text_StaticBG_Manpower extends Text_StaticBG
{
    ...
    public Text_StaticBG_Manpower(final String sText, final int fontID, final int iTextPositionX, final int iPosX, final int iPosY, final int iWidth, final int iHeight, final int id) {
    ...
    public void draw(final SpriteBatch oSB, final int iTranslateX, final int iTranslateY, final boolean isActive, final boolean scrollableY) {
        ...
        if (Game.getProvince(this.id).iProvinceIncreaseManpowerSize > 0) {
            ...
            Renderer.drawBoxProgress(oSB, Images.statsRectBG, this.getPosX() + iTranslateX, this.getPosY() + iTranslateY, (int)(this.getWidth() * (1.0f - Game.getProvince(this.id).provinceIncreaseManpowerDaysLeft.get(0).daysLeft / (float)Game.getProvince(this.id).provinceIncreaseManpowerDaysLeft.get(0).investTime)), this.getHeight(), this.getWidth());
        ...
        if (this.getClickable() && this.getIsHovered() && Text_StaticBG_Manpower.animationState >= 0) {
            if (Text_StaticBG_Manpower.animationState == 0) {
                final float drawPerc = Math.min(1.0f * (CFG.currentTimeMillis - Text_StaticBG_Manpower.lTimeAnimation) / 1000.0f, 1.0f);
                ...
                if (Text_StaticBG_Manpower.lTimeAnimation < CFG.currentTimeMillis - 1000L) {
                    ++Text_StaticBG_Manpower.animationState;
                    Text_StaticBG_Manpower.lTimeAnimation = CFG.currentTimeMillis;
                }
            }
            else {
                final float drawPerc = Math.min(1.0f * (CFG.currentTimeMillis - Text_StaticBG_Manpower.lTimeAnimation) / 1000.0f, 1.0f);
                ...
                if (Text_StaticBG_Manpower.lTimeAnimation < CFG.currentTimeMillis - 1000L) {
                    Text_StaticBG_Manpower.animationState = 0;
                    Text_StaticBG_Manpower.lTimeAnimation = CFG.currentTimeMillis;
                }
            }
            oSB.setColor(Color.WHITE);
    ...
    public String getTextToDraw() {
        if (this.lastValue != Game.getProvince(this.id).getManpower()) {
            this.setText("" + CFG.getPrecision2(Game.getProvince(this.id).getManpower(), 100));
            this.lastValue = Game.getProvince(this.id).getManpower();
        }
        return this.sText;
    }
    ...
    static {
        Text_StaticBG_Manpower.lTimeAnimation = 0L;
        Text_StaticBG_Manpower.animationState = 0;
    }
}
