...
public class MenuElement_HoverElement_Type_FlagCiv_Title implements MenuElement_HoverElement_Type
{
    ...
    public MenuElement_HoverElement_Type_FlagCiv_Title(final int iCivID, final String nText2) {
        this.iTextWidthMAX = 0;
        this.iCivID = iCivID;
        this.sText2 = nText2;
        Renderer.glyphLayout.setText(Renderer.fontMain.get(MenuElement_HoverElement_Type_FlagCiv_Title.fontID), Game.getCiv(iCivID).getCivName());
        this.iTextWidthMAX = (int)Renderer.glyphLayout.width;
        Renderer.glyphLayout.setText(Renderer.fontMain.get(MenuElement_HoverElement_Type_FlagCiv_Title.fontID2), this.sText2);
        this.iTextWidthMAX = Math.max(this.iTextWidthMAX, (int)Renderer.glyphLayout.width);
        if (iCivID < 0) {
            this.civName = Game.lang.get("Rebels");
        }
        else {
            this.civName = Game.getCiv(iCivID).getCivName();
        }
    }
    
    @Override
    public void draw(final SpriteBatch oSB, final int nPosX, final int nPosY, final float nAlpha, final int iMaxWidth) {
        oSB.setColor(Game.getCiv(this.iCivID).getRGB(0.65f * nAlpha));
