...
public class ButtonStatsRectIMG_Manpower extends Button
{
    public int imageID;
    ...
    public ButtonStatsRectIMG_Manpower(final int nProvinceID, final String sText, final int imageID, final int iPosX, final int iPosY, final int nWidth, final int nHeight, final int maxIconWidth) {
    ...
    protected void drawButtonBG(final SpriteBatch oSB, final int iTranslateX, final int iTranslateY, final boolean isActive) {
        ...
        try {
            if (Game.getProvince(this.iProvinceID).iProvinceIncreaseManpowerSize > 0) {
                ...
                Renderer.drawBoxProgress(oSB, Images.statsRectBG, this.getPosX() + iTranslateX, this.getPosY() + iTranslateY, (int)(this.getWidth() * (1.0f - Game.getProvince(this.iProvinceID).provinceIncreaseManpowerDaysLeft.get(0).daysLeft / (float)Game.getProvince(this.iProvinceID).provinceIncreaseManpowerDaysLeft.get(0).investTime)), this.getHeight(), this.getWidth());
        ...
        if (this.getClickable() && this.getIsHovered() && ButtonStatsRectIMG_Manpower.animationState >= 0) {
            if (ButtonStatsRectIMG_Manpower.animationState == 0) {
                final float drawPerc = Math.min(1.0f * (CFG.currentTimeMillis - ButtonStatsRectIMG_Manpower.lTimeAnimation) / 1000.0f, 1.0f);
                ...
                if (ButtonStatsRectIMG_Manpower.lTimeAnimation < CFG.currentTimeMillis - 1000L) {
                    ++ButtonStatsRectIMG_Manpower.animationState;
                    ButtonStatsRectIMG_Manpower.lTimeAnimation = CFG.currentTimeMillis;
                }
            }
            else {
                final float drawPerc = Math.min(1.0f * (CFG.currentTimeMillis - ButtonStatsRectIMG_Manpower.lTimeAnimation) / 1000.0f, 1.0f);
                ...
                if (ButtonStatsRectIMG_Manpower.lTimeAnimation < CFG.currentTimeMillis - 1000L) {
                    ButtonStatsRectIMG_Manpower.animationState = 0;
                    ButtonStatsRectIMG_Manpower.lTimeAnimation = CFG.currentTimeMillis;
                }
            }
            oSB.setColor(Color.WHITE);
    ...
    public String getTextToDraw() {
        if (this.lastValue != Game.getProvince(this.iProvinceID).getManpower()) {
            this.setText("" + CFG.getPrecision2(Game.getProvince(this.iProvinceID).getManpower(), 10));
            this.lastValue = Game.getProvince(this.iProvinceID).getManpower();
        }
    ...
    protected void drawText(final SpriteBatch oSB, final int iTranslateX, final int iTranslateY, final boolean isActive) {
        if (this.getIsHovered() && (Game.getProvince(this.iProvinceID).getCivID() == Game.player.iCivID || Game.getCiv(Game.getProvince(this.iProvinceID).getCivID()).getPuppetOfCivID() == Game.player.iCivID)) {
            ImageManager.getImage(Game_Calendar.IMG_MANPOWER_UP).draw(oSB, this.getPosX() + CFG.PADDING + this.maxIconWidth / 2 - this.iconWidth / 2 + iTranslateX, this.getPosY() + (this.getHeight() - this.iconHeight) / 2 + iTranslateY, this.iconWidth, this.iconHeight);
        }
        else {
    ...
    static {
        ButtonStatsRectIMG_Manpower.lTimeAnimation = 0L;
        ButtonStatsRectIMG_Manpower.animationState = 0;
    }
}
