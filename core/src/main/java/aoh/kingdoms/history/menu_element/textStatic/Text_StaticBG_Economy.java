...
public class Text_StaticBG_Economy extends Text_StaticBG
{
    ...
    public float percOfMax;
    public static Color progressBarBG;
    public static Color progressBar;
    
    public Text_StaticBG_Economy(final String sText, final int fontID, final int iTextPositionX, final int iPosX, final int iPosY, final int iWidth, final int iHeight, final int id) {
    ...
    public void draw(final SpriteBatch oSB, final int iTranslateX, final int iTranslateY, final boolean isActive, final boolean scrollableY) {
        ...
        if (Game.getProvince(this.id).iProvinceInvestSize > 0) {
            ...
            Renderer.drawBoxProgress(oSB, Images.statsRectBG, this.getPosX() + iTranslateX, this.getPosY() + iTranslateY, (int)(this.getWidth() * (1.0f - Game.getProvince(this.id).provinceInvestDaysLeft.get(0).daysLeft / (float)Game.getProvince(this.id).provinceInvestDaysLeft.get(0).investTime)), this.getHeight(), this.getWidth());
        ...
        oSB.setColor(Text_StaticBG_Economy.progressBarBG);
        Renderer.drawBox(oSB, Images.statsRectBG, this.getPosX() + iTranslateX, this.getPosY() + this.getHeight() - CFG.PADDING + iTranslateY, this.getWidth(), CFG.PADDING, 1.0f);
        oSB.setColor(Text_StaticBG_Economy.progressBar);
        Renderer.drawBox(oSB, Images.statsRectBG, this.getPosX() + iTranslateX, this.getPosY() + this.getHeight() - CFG.PADDING + iTranslateY, (int)(this.getWidth() * this.percOfMax), CFG.PADDING, 1.0f);
        oSB.setColor(Color.WHITE);
        if (this.getClickable() && this.getIsHovered() && Text_StaticBG_Economy.animationState >= 0) {
            if (Text_StaticBG_Economy.animationState == 0) {
                final float drawPerc = Math.min(1.0f * (CFG.currentTimeMillis - Text_StaticBG_Economy.lTimeAnimation) / 1000.0f, 1.0f);
                ...
                if (Text_StaticBG_Economy.lTimeAnimation < CFG.currentTimeMillis - 1000L) {
                    ++Text_StaticBG_Economy.animationState;
                    Text_StaticBG_Economy.lTimeAnimation = CFG.currentTimeMillis;
                }
            }
            else {
                final float drawPerc = Math.min(1.0f * (CFG.currentTimeMillis - Text_StaticBG_Economy.lTimeAnimation) / 1000.0f, 1.0f);
                ...
                if (Text_StaticBG_Economy.lTimeAnimation < CFG.currentTimeMillis - 1000L) {
                    Text_StaticBG_Economy.animationState = 0;
                    Text_StaticBG_Economy.lTimeAnimation = CFG.currentTimeMillis;
                }
            }
            oSB.setColor(Color.WHITE);
    ...
    public String getTextToDraw() {
        if (this.lastValue != Game.getProvince(this.id).getEconomyWithBonuses()) {
            this.setText("" + CFG.getPrecision2(Game.getProvince(this.id).getEconomyWithBonuses(), 100));
            this.lastValue = Game.getProvince(this.id).getEconomyWithBonuses();
            this.percOfMax = Math.min(1.0f, Math.max(0.15f, Game.getProvince(this.id).getEconomyWithBonuses() / Game.getMaxEconomy(this.id)));
        }
        return this.sText;
    }
    ...
    static {
        Text_StaticBG_Economy.lTimeAnimation = 0L;
        Text_StaticBG_Economy.animationState = 0;
        Text_StaticBG_Economy.progressBarBG = new Color(0.09803922f, 0.09803922f, 0.15686275f, 0.5f);
        Text_StaticBG_Economy.progressBar = new Color(0.0f, 0.60784316f, 0.3529412f, 0.5f);
    }
}
