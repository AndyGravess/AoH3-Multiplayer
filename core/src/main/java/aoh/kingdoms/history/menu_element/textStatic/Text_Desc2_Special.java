...
{
    public Text_Desc2_Special(final String sText, final int iPosX, final int iPosY, final int iWidth) {
        super(sText, iPosX, iPosY, iWidth);
    }
    
    public Text_Desc2_Special(final String sText, final int iPosX, final int iPosY, final int iWidth, final int fontID) {
    ...
    public void draw(final SpriteBatch oSB, final int iTranslateX, final int iTranslateY, final boolean isActive, final boolean scrollableY) {
        ...
        for (int i = 0; i < this.iLineSize; ++i) {
            Renderer.drawText(oSB, this.fontID, this.sLines.get(i), this.getPosX() + this.getPadding() + iTranslateX, this.getPosY() + this.getPaddingY() + (this.iTextHeight + CFG.PADDING * 2) * i + iTranslateY, this.getColor(isActive));
        }
    }
