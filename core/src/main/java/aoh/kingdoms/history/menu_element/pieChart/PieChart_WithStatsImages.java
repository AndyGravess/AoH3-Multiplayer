...
public class PieChart_WithStatsImages extends PieChart_WithStats
{
    public PieChart_WithStatsImages(final int iPosX, final int iPosY, final int iWidth, final int iHeight, final PieChart_Data nPieChartData, final MenuElement_Hover menuElementHover) {
    ...
    public Image getStatImage(final int nID) {
        return ImageManager.getImage(Images.time);
    }
    
    public int getStatsImageWidth() {
        return this.getStatImage(0).getWidth();
    }
    
    public int getStatsImageHeight() {
        return this.getStatImage(0).getHeight();
    }
        ...
        if (this.isDescriptionActive || this.hideAnimation) {
            ...
            for (int i = 0; i < this.pieChartData.getPieChartValuesSize(); ++i) {
                try {
                    ...
                    this.getStatImage(this.pieChartData.getPieChartValue(i).getDataID()).draw(oSB, nPosX + nWidth_LEFT + CFG.PADDING * 2 + CFG.COLOR_WIDTH + iTranslateX, nPosY + (int)(CFG.TEXT_HEIGHT * this.TEXT_SCALE + CFG.PADDING * 2) * i + CFG.PADDING * i + (int)(CFG.TEXT_HEIGHT * this.TEXT_SCALE + CFG.PADDING * 2) / 2 - (int)(CFG.TEXT_HEIGHT * this.TEXT_SCALE / 2.0f) + iTranslateY, (int)(this.getStatsImageWidth() * tempFlagScale), (int)(this.getStatsImageHeight() * tempFlagScale));
                }
                catch (final IndexOutOfBoundsException ex) {
