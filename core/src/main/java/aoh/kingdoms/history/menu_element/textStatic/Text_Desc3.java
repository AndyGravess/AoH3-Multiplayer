...
public class Text_Desc3 extends Text_Static
{
    public List<String> sLines;
    public int iLineSize;
    
    public Text_Desc3(final String sText, final int iPosX, final int iPosY, final int iWidth, final int iHeight) {
        ...
        if (this.sLines.size() > 0 && this.sLines.get(0).length() > 0) {
            ...
            glyphLayout.setText(Renderer.fontMain.get(this.fontID), "ABC");
        ...
        for (int i = 0; i < this.iLineSize; ++i) {
            ...
            glyphLayout2.setText(Renderer.fontMain.get(this.fontID), this.sLines.get(i));
            if (glyphLayout2.width > this.getWidth()) {
                this.setWidth((int)glyphLayout2.width);
            }
        }
        this.setHeight(Math.max(iHeight, this.iTextHeight * this.sLines.size() + (this.sLines.size() - 1) * CFG.PADDING * 2 + this.getPaddingY() * 2));
    }
    ...
    public void draw(final SpriteBatch oSB, final int iTranslateX, final int iTranslateY, final boolean isActive, final boolean scrollableY) {
        ...
        final int paddingY = (this.getHeight() - (this.iTextHeight * this.sLines.size() + (this.sLines.size() - 1) * CFG.PADDING * 2)) / 2;
        for (int i = 0; i < this.iLineSize; ++i) {
            Renderer.drawTextWithShadow(oSB, this.fontID, this.sLines.get(i), this.getPosX() + this.getPadding() + iTranslateX, this.getPosY() + paddingY + (this.iTextHeight + CFG.PADDING * 2) * i + iTranslateY, this.getColor(isActive));
        }
    }
