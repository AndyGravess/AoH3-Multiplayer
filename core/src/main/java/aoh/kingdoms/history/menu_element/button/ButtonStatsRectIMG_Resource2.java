...
public class ButtonStatsRectIMG_Resource2 extends Button
{
    public int imageID;
    ...
    public ButtonStatsRectIMG_Resource2(final String sText, final int imageID, final int iPosX, final int iPosY, final int nWidth, final int nHeight, final int maxIconWidth, final int id) {
        this.init(sText, CFG.FONT_REGULAR_SMALL, 0, iPosX, iPosY, nWidth, nHeight, true, true, false, false);
        this.imageID = imageID;
        this.id = id;
        this.maxIconWidth = maxIconWidth;
        final float iconScale = Math.min(1.0f, this.getImageScale(imageID));
    ...
    protected void drawText(final SpriteBatch oSB, final int iTranslateX, final int iTranslateY, final boolean isActive) {
        ...
        Renderer.drawTextWithShadow(oSB, this.fontID, this.getTextToDraw(), this.getPosX() + CFG.PADDING * 2 + this.maxIconWidth + CFG.PADDING + iTranslateX, this.getPosY() + this.getHeight() / 2 - this.iTextHeight / 2 + iTranslateY, this.getColor(isActive));
    }
    
    private final float getImageScale(final int iImageID) {
        return (this.getHeight() - CFG.PADDING * 2) / (float)ResourcesManager.resourceImages.get(iImageID).getHeight();
    }
