...
public class Text_StaticBG_ID_Flag extends Text_StaticBG
{
    public int id;
    
    public Text_StaticBG_ID_Flag(final String sText, final int fontID, final int iTextPositionX, final int iPosX, final int iPosY, final int iWidth, final int iHeight, final int id) {
    ...
    public Text_StaticBG_ID_Flag(final String sText, final int fontID, final int iTextPositionX, final int iPosX, final int iPosY, final int iWidth, final int iHeight, final int id, final boolean flag) {
        super(sText, fontID, iTextPositionX, iPosX, iPosY, iWidth, iHeight);
        this.id = id;
    ...
    public void draw(final SpriteBatch oSB, final int iTranslateX, final int iTranslateY, final boolean isActive, final boolean scrollableY) {
        ...
        Images.gradientFull.draw(oSB, this.getPosX() + iTranslateX, this.getPosY() + 1 + iTranslateY, this.getWidth(), 1);
        oSB.setColor(Color.WHITE);
        Game.getCiv(this.getFlagID()).getFlag().draw(oSB, this.getPosX() + iTranslateX + CFG.PADDING * 2, this.getPosY() + this.getHeight() / 2 - ImageManager.getImage(Images.flag_rect).getHeight() / 2 + iTranslateY, ImageManager.getImage(Images.flag_rect).getWidth(), ImageManager.getImage(Images.flag_rect).getHeight());
    ...
    public int getFlagID() {
        return Game.getProvince(this.id).getCivID();
    }
    ...
    public void setIsHovered(final boolean isHovered) {
        ...
        if (isHovered) {
            ProvinceDraw.drawProvincesCiv_HoveredFlagID = Game.getProvince(this.id).getCivID();
        }
    }
    
    @Override
    public void actionElementPPM() {
        if (this.getFlagID() > 0 && Game.getCiv(this.getFlagID()).getCapitalProvinceID() >= 0 && this.getFlagID() == Game.getProvince(Game.getCiv(this.getFlagID()).getCapitalProvinceID()).getCivID()) {
            Game.mapCoords.centerToProvinceID(Game.getCiv(this.getFlagID()).getCapitalProvinceID());
        }
    }
}
