...
public class MenuElement_HoverElement_Type_Button_TextBonusIdeology implements MenuElement_HoverElement_Type
{
    ...
    public MenuElement_HoverElement_Type_Button_TextBonusIdeology(final String sText, final String sText2, final int imageID, final int fontID, final Color nColorBonus) {
        this.fontID = 0;
        this.fontID2 = 0;
        this.init(sText, sText2, imageID, fontID, CFG.FONT_BOLD_SMALL, Colors.BUTTON_TEXT_BRIGHT, nColorBonus);
    }
    
    public MenuElement_HoverElement_Type_Button_TextBonusIdeology(final String sText, final String sText2, final int imageID, final int fontID, final int fontID2, final Color nColorBonus) {
        this.fontID = 0;
        this.fontID2 = 0;
        this.init(sText, sText2, imageID, fontID, fontID2, Colors.BUTTON_TEXT_BRIGHT, nColorBonus);
    }
    
    public MenuElement_HoverElement_Type_Button_TextBonusIdeology(final String sText, final String sText2, final int imageID, final int fontID, final int fontID2, final Color nColorLeft, final Color nColorBonus) {
        this.fontID = 0;
        this.fontID2 = 0;
    ...
    public final void init(final String sText, final String sText2, final int imageID, final int nFontID, final int fontID2, final Color oColor, final Color nColorBonus) {
        ...
        this.iconWidth = (int)(Game.ideologiesManager.ideologiesImages.get(imageID).getWidth() * iconScale);
        this.iconHeight = (int)(Game.ideologiesManager.ideologiesImages.get(imageID).getHeight() * iconScale);
    }
    ...
    public void draw(final SpriteBatch oSB, final int nPosX, int nPosY, final float nAlpha, final int iMaxWidth) {
        ...
        Game.ideologiesManager.ideologiesImages.get(this.imageID).draw(oSB, nPosX + (this.maxIconWidth + CFG.PADDING * 2) / 2 - this.iconWidth / 2, nPosY + (this.getHeight2() - this.iconHeight) / 2, this.iconWidth, this.iconHeight);
    ...
    private final float getImageScale(final int iImageID) {
        return CFG.TEXT_HEIGHT / (float)Game.ideologiesManager.ideologiesImages.get(iImageID).getHeight();
    }
}
