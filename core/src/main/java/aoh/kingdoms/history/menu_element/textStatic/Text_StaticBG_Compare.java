...
public class Text_StaticBG_Compare extends Text_StaticBG_ID
{
    public String textLeft;
    public int iconLeft;
    public boolean left;
    
    public Text_StaticBG_Compare(final boolean left, final String textLeft, final int iconLeft, final String sText, final int iPosX, final int iPosY, final int iWidth, final int iHeight, final boolean id) {
        super(sText, id ? CFG.FONT_BOLD_SMALL : CFG.FONT_REGULAR_SMALL, -1, iPosX, iPosY, iWidth, iHeight, id ? 1 : 0);
        this.left = left;
    ...
    public void drawText(final SpriteBatch oSB, final int iTranslateX, final int iTranslateY, final boolean isActive, final boolean scrollableY) {
        Renderer.drawText(oSB, this.fontID, this.sText, this.getPosX() + this.textPosition.getTextPosition() + iTranslateX, this.getPosY() + (this.getHeight() - CFG.TEXT_HEIGHT) / 2 + iTranslateY, this.getColor(isActive));
    }
    ...
    public void buildElementHover() {
        ...
        if (this.left) {
            nData.add(new MenuElement_HoverElement_Type_Button_TextBonusFlag("", Game.getCiv(InGame_Civ_Compare.civLeft).getCivName(), InGame_Civ_Compare.civLeft, CFG.FONT_BOLD_SMALL, Colors.HOVER_GOLD));
            nElements.add(new MenuElement_HoverElement(nData));
            nData.clear();
        }
        else {
            nData.add(new MenuElement_HoverElement_Type_Button_TextBonusFlag("", Game.getCiv(InGame_Civ_Compare.civRight).getCivName(), InGame_Civ_Compare.civRight, CFG.FONT_BOLD_SMALL, Colors.HOVER_GOLD));
            nElements.add(new MenuElement_HoverElement(nData));
            nData.clear();
        }
        nData.add(new MenuElement_HoverElement_Type_Button_TextBonus(this.textLeft, this.getText(), this.iconLeft, CFG.FONT_REGULAR_SMALL, CFG.FONT_BOLD_SMALL, Colors.HOVER_LEFT, Colors.HOVER_GOLD));
        nElements.add(new MenuElement_HoverElement(nData));
        nData.clear();
