...
public class TextIcon2_Infrastructure extends Button
{
    ...
    public TextIcon2_Infrastructure(final int nProvinceID, final String sText, final int imageID, final int nPosX, final int nPosY, final int nWidth, final int nHeight) {
    ...
    protected void drawText(final SpriteBatch oSB, final int iTranslateX, final int iTranslateY, final boolean isActive) {
        ...
        if (Game.getProvince(this.iProvinceID).iProvinceDevelopInfrastructureSize > 0) {
        ...
        try {
            if (Game.getProvince(this.iProvinceID).iProvinceDevelopInfrastructureSize > 0) {
                ...
                Renderer.drawBoxProgress(oSB, Images.statsRectBG, this.getPosX() + iTranslateX, this.getPosY() + this.getHeight() - this.getTextH() + iTranslateY, (int)(this.getWidth() * (1.0f - Game.getProvince(this.iProvinceID).provinceDevelopInfrastructureDaysLeft.get(0).daysLeft / (float)Game.getProvince(this.iProvinceID).provinceDevelopInfrastructureDaysLeft.get(0).investTime)), this.getTextH(), this.getWidth());
                oSB.setColor(Color.WHITE);
            }
        }
        ...
        if (this.getClickable() && this.getIsHovered() && TextIcon2_Infrastructure.animationState >= 0) {
            if (TextIcon2_Infrastructure.animationState == 0) {
                final float drawPerc = Math.min(1.0f * (CFG.currentTimeMillis - TextIcon2_Infrastructure.lTimeAnimation) / 1000.0f, 1.0f);
                ...
                if (TextIcon2_Infrastructure.lTimeAnimation < CFG.currentTimeMillis - 1000L) {
                    ++TextIcon2_Infrastructure.animationState;
                    TextIcon2_Infrastructure.lTimeAnimation = CFG.currentTimeMillis;
                }
            }
            else {
                final float drawPerc = Math.min(1.0f * (CFG.currentTimeMillis - TextIcon2_Infrastructure.lTimeAnimation) / 1000.0f, 1.0f);
                ...
                if (TextIcon2_Infrastructure.lTimeAnimation < CFG.currentTimeMillis - 1000L) {
                    TextIcon2_Infrastructure.animationState = 0;
                    TextIcon2_Infrastructure.lTimeAnimation = CFG.currentTimeMillis;
                }
            }
            oSB.setColor(Color.WHITE);
        ...
        if (this.getIsHovered() && Game.getProvince(this.iProvinceID).getCivID() == Game.player.iCivID) {
            ImageManager.getImage(Images.infrastructureUp).draw(oSB, this.getPosX() + this.getWidth() / 2 - ImageManager.getImage(Images.infrastructureUp).getWidth() / 2 + iTranslateX, this.getPosY() + (this.getHeight() - this.getTextH()) / 2 - ImageManager.getImage(Images.infrastructureUp).getHeight() / 2 + iTranslateY);
        }
        else {
    ...
    public String getTextToDraw() {
        if (this.lastValue != Game.getProvince(this.iProvinceID).getInfrastructure() || this.lastValue2 != Game.getProvince(this.iProvinceID).iInfrastructureMax) {
            this.setText("" + Game.getProvince(this.iProvinceID).getInfrastructure() + " / " + Game.getProvince(this.iProvinceID).iInfrastructureMax);
            this.lastValue = Game.getProvince(this.iProvinceID).getInfrastructure();
            this.lastValue2 = Game.getProvince(this.iProvinceID).iInfrastructureMax;
        }
    ...
    static {
        TextIcon2_Infrastructure.lTimeAnimation = 0L;
        TextIcon2_Infrastructure.animationState = 0;
    }
}
