...
public class TextIcon2_Economy extends Button
{
    ...
    public float economyPercOfMax;
    
    public TextIcon2_Economy(final int nProvinceID, final String sText, final int imageID, final int nPosX, final int nPosY, final int nWidth, final int nHeight) {
        this.lastValue = -997654.3f;
        this.iProvinceID = nProvinceID;
        this.imageID = imageID;
        this.economyPercOfMax = Math.max(0.05f, Math.min(1.0f, Game.getProvince(this.iProvinceID).getEconomyWithBonuses() / Game.getMaxEconomy(this.iProvinceID)));
    ...
    protected void drawText(final SpriteBatch oSB, final int iTranslateX, final int iTranslateY, final boolean isActive) {
        ...
        if (Game.getProvince(this.iProvinceID).iProvinceInvestSize > 0) {
        ...
        Renderer.drawBoxProgress(oSB, Images.statsRectBG, this.getPosX() + iTranslateX, this.getPosY() + this.getHeight() - this.getTextH() + iTranslateY, (int)(this.getWidth() * this.economyPercOfMax), this.getTextH(), this.getWidth());
        ...
        try {
            if (Game.getProvince(this.iProvinceID).iProvinceInvestSize > 0) {
                ...
                Renderer.drawBoxProgress(oSB, Images.statsRectBG, this.getPosX() + iTranslateX, this.getPosY() + this.getHeight() - this.getTextH() + iTranslateY, (int)(this.getWidth() * (1.0f - Game.getProvince(this.iProvinceID).provinceInvestDaysLeft.get(0).daysLeft / (float)Game.getProvince(this.iProvinceID).provinceInvestDaysLeft.get(0).investTime)), this.getTextH(), this.getWidth());
                oSB.setColor(Color.WHITE);
            }
        }
        ...
        if (this.getClickable() && this.getIsHovered() && TextIcon2_Economy.animationState >= 0) {
            if (TextIcon2_Economy.animationState == 0) {
                final float drawPerc = Math.min(1.0f * (CFG.currentTimeMillis - TextIcon2_Economy.lTimeAnimation) / 1000.0f, 1.0f);
                ...
                if (TextIcon2_Economy.lTimeAnimation < CFG.currentTimeMillis - 1000L) {
                    ++TextIcon2_Economy.animationState;
                    TextIcon2_Economy.lTimeAnimation = CFG.currentTimeMillis;
                }
            }
            else {
                final float drawPerc = Math.min(1.0f * (CFG.currentTimeMillis - TextIcon2_Economy.lTimeAnimation) / 1000.0f, 1.0f);
                ...
                if (TextIcon2_Economy.lTimeAnimation < CFG.currentTimeMillis - 1000L) {
                    TextIcon2_Economy.animationState = 0;
                    TextIcon2_Economy.lTimeAnimation = CFG.currentTimeMillis;
                }
            }
            oSB.setColor(Color.WHITE);
        ...
        if (this.getIsHovered() && (Game.getProvince(this.iProvinceID).getCivID() == Game.player.iCivID || Game.getCiv(Game.getProvince(this.iProvinceID).getCivID()).getPuppetOfCivID() == Game.player.iCivID)) {
            ImageManager.getImage(Game_Calendar.IMG_ECONOMY_UP).draw(oSB, this.getPosX() + this.getWidth() / 2 - ImageManager.getImage(Game_Calendar.IMG_ECONOMY_UP).getWidth() / 2 + iTranslateX, this.getPosY() + (this.getHeight() - this.getTextH()) / 2 - ImageManager.getImage(Game_Calendar.IMG_ECONOMY_UP).getHeight() / 2 + iTranslateY);
        }
        else {
    ...
    public String getTextToDraw() {
        if (this.lastValue != Game.getProvince(this.iProvinceID).getEconomyWithBonuses()) {
            this.setText("" + CFG.getPrecision2(Game.getProvince(this.iProvinceID).getEconomyWithBonuses(), 10));
    ...
    static {
        TextIcon2_Economy.lTimeAnimation = 0L;
        TextIcon2_Economy.animationState = 0;
    }
}
