...
public class ButtonAdvisorGeneral extends Button
{
    ...
    public ButtonAdvisorGeneral(final int iPosX, final int iPosY, final String sName, final int imageID, final int iCivID, final int iHireModeID, final String sIMG) {
        this.iHireModeID = -1;
        ...
        try {
            ...
            if (this.advisorImage == null) {
                if (FileManager.loadFile("game/generals/" + GeneralManager.getGeneralsImgPath(iCivID) + CFG.getRescouresPath_Short() + Game.getCiv(iCivID).iGroupID + "/" + imageID + ".png").exists()) {
                    this.advisorImage = new Image(ImageManager.loadTexture_RGB888("game/generals/" + GeneralManager.getGeneralsImgPath(iCivID) + CFG.getRescouresPath_Short() + Game.getCiv(iCivID).iGroupID + "/" + imageID + ".png"), Texture.TextureFilter.Linear, Texture.TextureWrap.ClampToEdge);
                }
                else {
                    this.advisorImage = new Image(ImageManager.loadTexture_RGB888("game/generals/" + GeneralManager.getGeneralsImgPath(iCivID) + CFG.getRescouresPath_Short() + Game.getCiv(iCivID).iGroupID + "/0.png"), Texture.TextureFilter.Linear, Texture.TextureWrap.ClampToEdge);
                }
            }
        }
    ...
    public void buildElementHover() {
        ...
        if (this.iHireModeID >= 0) {
            ...
            list.add(new MenuElement_HoverElement_Type_Text(AdvisorManager.getAdvisorGroupName(3), CFG.FONT_REGULAR_SMALL));
            nElements.add(new MenuElement_HoverElement(nData));
            nData.clear();
            ...
            list2.add(new MenuElement_HoverElement_Type_Text(AdvisorManager.getAdvisorGroupName(3), CFG.FONT_REGULAR_SMALL));
            nElements.add(new MenuElement_HoverElement(nData));
            nData.clear();
