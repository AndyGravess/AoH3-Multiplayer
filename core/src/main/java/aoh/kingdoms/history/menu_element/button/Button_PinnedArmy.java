...
public class Button_PinnedArmy extends Button
{
    public String key;
    ...
    public Button_PinnedArmy(final String key, final int iPosX, final int iPosY) {
        this.key = null;
            ...
            if (Game_Calendar.TURN_ID != this.iLastTurnID) {
                ...
                if (nArmyPos != null) {
                    if (Game.getProvince(nArmyPos.iProvinceID).getArmy(nArmyPos.iID).armyGeneral == null) {
                        ...
                        if (this.generalImage == null) {
                            if (FileManager.loadFile("game/generals/" + GeneralManager.getGeneralsImgPath(Game.player.iCivID) + CFG.getRescouresPath_Short() + Game.getCiv(Game.player.iCivID).iGroupID + "/" + this.iActiveGeneralIMG_ID + ".png").exists()) {
                                this.generalImage = new Image(ImageManager.loadTexture_RGB888("game/generals/" + GeneralManager.getGeneralsImgPath(Game.player.iCivID) + CFG.getRescouresPath_Short() + Game.getCiv(Game.player.iCivID).iGroupID + "/" + this.iActiveGeneralIMG_ID + ".png"), Texture.TextureFilter.Linear, Texture.TextureWrap.ClampToEdge);
                            }
                            else {
                                this.generalImage = new Image(ImageManager.loadTexture_RGB888("game/generals/" + GeneralManager.getGeneralsImgPath(Game.player.iCivID) + CFG.getRescouresPath_Short() + Game.getCiv(Game.player.iCivID).iGroupID + "/0.png"), Texture.TextureFilter.Linear, Texture.TextureWrap.ClampToEdge);
                            }
                        }
                    }
                    if (this.iArmyNum != Game.getProvince(nArmyPos.iProvinceID).getArmy(nArmyPos.iID).iArmy) {
                        ...
                        this.setText(Game.getProvince(nArmyPos.iProvinceID).getArmy(nArmyPos.iID).sArmy);
                    }
                }
                else {
                    Game.player.removePinArmy(this.key);
                }
            }
        }
    ...
    public void buildElementHover() {
        ...
        if (nArmyPos != null) {
            ...
            if (Game.getProvince(nArmyPos.iProvinceID).getArmy(nArmyPos.iID).armyGeneral != null) {
                nData.add(new MenuElement_HoverElement_Type_Button_TextBonus(Game.getProvince(nArmyPos.iProvinceID).getArmy(nArmyPos.iID).armyGeneral.n, "", Images.general, CFG.FONT_REGULAR_SMALL, CFG.FONT_REGULAR_SMALL, Colors.HOVER_LEFT, Colors.HOVER_LEFT));
                nElements.add(new MenuElement_HoverElement(nData));
                nData.clear();
            }
            ...
            for (int i = 0; i < Game.getProvince(nArmyPos.iProvinceID).getArmy(nArmyPos.iID).iArmyRegimentSize; ++i) {
                if (ArmyManager.lUnitsTypes.get(Game.getProvince(nArmyPos.iProvinceID).getArmy(nArmyPos.iID).lArmyRegiment.get(i).uID).Line < 2) {
                    ...
                    nData.add(new MenuElement_HoverElement_Type_BattleArmy("" + tUnits, Game.getProvince(nArmyPos.iProvinceID).getArmy(nArmyPos.iID).lArmyRegiment.get(i).uID, Game.getProvince(nArmyPos.iProvinceID).getArmy(nArmyPos.iID).lArmyRegiment.get(i).aID, (i > 0) ? CFG.PADDING : 0));
                }
            }
            for (int i = 0; i < Game.getProvince(nArmyPos.iProvinceID).getArmy(nArmyPos.iID).iArmyRegimentSize; ++i) {
                if (ArmyManager.lUnitsTypes.get(Game.getProvince(nArmyPos.iProvinceID).getArmy(nArmyPos.iID).lArmyRegiment.get(i).uID).Line >= 2) {
