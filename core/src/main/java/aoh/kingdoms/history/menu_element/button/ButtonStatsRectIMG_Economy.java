...
public class ButtonStatsRectIMG_Economy extends Button
{
    public int imageID;
    ...
    public ButtonStatsRectIMG_Economy(final int nProvinceID, final String sText, final int imageID, final int iPosX, final int iPosY, final int nWidth, final int nHeight, final int maxIconWidth) {
    ...
    protected void drawButtonBG(final SpriteBatch oSB, final int iTranslateX, final int iTranslateY, final boolean isActive) {
        ...
        try {
            if (Game.getProvince(this.iProvinceID).iProvinceInvestSize > 0) {
                ...
                Renderer.drawBoxProgress(oSB, Images.statsRectBG, this.getPosX() + iTranslateX, this.getPosY() + iTranslateY, (int)(this.getWidth() * (1.0f - Game.getProvince(this.iProvinceID).provinceInvestDaysLeft.get(0).daysLeft / (float)Game.getProvince(this.iProvinceID).provinceInvestDaysLeft.get(0).investTime)), this.getHeight(), this.getWidth());
        ...
        if (this.getClickable() && this.getIsHovered() && ButtonStatsRectIMG_Economy.animationState >= 0) {
            if (ButtonStatsRectIMG_Economy.animationState == 0) {
                final float drawPerc = Math.min(1.0f * (CFG.currentTimeMillis - ButtonStatsRectIMG_Economy.lTimeAnimation) / 1000.0f, 1.0f);
                ...
                if (ButtonStatsRectIMG_Economy.lTimeAnimation < CFG.currentTimeMillis - 1000L) {
                    ++ButtonStatsRectIMG_Economy.animationState;
                    ButtonStatsRectIMG_Economy.lTimeAnimation = CFG.currentTimeMillis;
                }
            }
            else {
                final float drawPerc = Math.min(1.0f * (CFG.currentTimeMillis - ButtonStatsRectIMG_Economy.lTimeAnimation) / 1000.0f, 1.0f);
                ...
                if (ButtonStatsRectIMG_Economy.lTimeAnimation < CFG.currentTimeMillis - 1000L) {
                    ButtonStatsRectIMG_Economy.animationState = 0;
                    ButtonStatsRectIMG_Economy.lTimeAnimation = CFG.currentTimeMillis;
                }
            }
            oSB.setColor(Color.WHITE);
    ...
    protected void drawText(final SpriteBatch oSB, final int iTranslateX, final int iTranslateY, final boolean isActive) {
        if (this.getIsHovered() && (Game.getProvince(this.iProvinceID).getCivID() == Game.player.iCivID || Game.getCiv(Game.getProvince(this.iProvinceID).getCivID()).getPuppetOfCivID() == Game.player.iCivID)) {
            ImageManager.getImage(Game_Calendar.IMG_ECONOMY_UP).draw(oSB, this.getPosX() + CFG.PADDING + this.maxIconWidth / 2 - this.iconWidth / 2 + iTranslateX, this.getPosY() + (this.getHeight() - this.iconHeight) / 2 + iTranslateY, this.iconWidth, this.iconHeight);
        }
        else {
    ...
    static {
        ButtonStatsRectIMG_Economy.lTimeAnimation = 0L;
        ButtonStatsRectIMG_Economy.animationState = 0;
    }
}
