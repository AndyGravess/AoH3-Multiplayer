...
public class PieChart_WithStatsGovernment extends PieChart_WithStats
{
    public PieChart_WithStatsGovernment(final int iPosX, final int iPosY, final int iWidth, final int iHeight, final PieChart_Data nPieChartData, final MenuElement_Hover menuElementHover) {
    ...
    public Image getStatImage(final int nID) {
        return Game.ideologiesManager.ideologiesImages.get(nID);
    }
    
    public int getStatsImageWidth() {
        return Game.ideologiesManager.ideologiesImages.get(0).getWidth();
    }
    
    public int getStatsImageHeight() {
        return Game.ideologiesManager.ideologiesImages.get(0).getHeight();
    }
    
    public int getStatsImageWidth(final int nID) {
        return Game.ideologiesManager.ideologiesImages.get(nID).getWidth();
    }
    
    public int getStatsImageHeight(final int nID) {
        return Game.ideologiesManager.ideologiesImages.get(nID).getHeight();
    }
        ...
        if (this.isDescriptionActive || this.hideAnimation) {
            ...
            for (int i = 0; i < this.pieChartData.getPieChartValuesSize(); ++i) {
                try {
                    ...
                    this.getStatImage(this.pieChartData.getPieChartValue(i).getDataID()).draw(oSB, nPosX + nWidth_LEFT + CFG.PADDING * 2 + CFG.COLOR_WIDTH + iTranslateX, nPosY + (int)(CFG.TEXT_HEIGHT * this.TEXT_SCALE + CFG.PADDING * 2) * i + CFG.PADDING * i - CFG.PADDING / 2 + (int)(CFG.TEXT_HEIGHT * this.TEXT_SCALE + CFG.PADDING * 2) / 2 - (int)(CFG.TEXT_HEIGHT * this.TEXT_SCALE / 2.0f) + iTranslateY, (int)(Game.ideologiesManager.ideologiesImages.get(this.pieChartData.getPieChartValue(i).getDataID()).getWidth() * tempFlagScale), (int)(Game.ideologiesManager.ideologiesImages.get(this.pieChartData.getPieChartValue(i).getDataID()).getHeight() * tempFlagScale));
                }
                catch (final IndexOutOfBoundsException ex) {
