...
public class TextIcon2_Manpower extends Button
{
    ...
    public TextIcon2_Manpower(final int nProvinceID, final String sText, final int imageID, final int nPosX, final int nPosY, final int nWidth, final int nHeight) {
    ...
    protected void drawText(final SpriteBatch oSB, final int iTranslateX, final int iTranslateY, final boolean isActive) {
        ...
        try {
            if (Game.getProvince(this.iProvinceID).iProvinceIncreaseManpowerSize > 0) {
                ...
                Renderer.drawBoxProgress(oSB, Images.statsRectBG, this.getPosX() + iTranslateX, this.getPosY() + this.getHeight() - this.getTextH() + iTranslateY, (int)(this.getWidth() * (1.0f - Game.getProvince(this.iProvinceID).provinceIncreaseManpowerDaysLeft.get(0).daysLeft / (float)Game.getProvince(this.iProvinceID).provinceIncreaseManpowerDaysLeft.get(0).investTime)), this.getTextH(), this.getWidth());
                oSB.setColor(Color.WHITE);
            }
        }
        ...
        if (this.getClickable() && this.getIsHovered() && TextIcon2_Manpower.animationState >= 0) {
            if (TextIcon2_Manpower.animationState == 0) {
                final float drawPerc = Math.min(1.0f * (CFG.currentTimeMillis - TextIcon2_Manpower.lTimeAnimation) / 1000.0f, 1.0f);
                ...
                if (TextIcon2_Manpower.lTimeAnimation < CFG.currentTimeMillis - 1000L) {
                    ++TextIcon2_Manpower.animationState;
                    TextIcon2_Manpower.lTimeAnimation = CFG.currentTimeMillis;
                }
            }
            else {
                final float drawPerc = Math.min(1.0f * (CFG.currentTimeMillis - TextIcon2_Manpower.lTimeAnimation) / 1000.0f, 1.0f);
                ...
                if (TextIcon2_Manpower.lTimeAnimation < CFG.currentTimeMillis - 1000L) {
                    TextIcon2_Manpower.animationState = 0;
                    TextIcon2_Manpower.lTimeAnimation = CFG.currentTimeMillis;
                }
            }
            oSB.setColor(Color.WHITE);
        ...
        if (this.getIsHovered() && (Game.getProvince(this.iProvinceID).getCivID() == Game.player.iCivID || Game.getCiv(Game.getProvince(this.iProvinceID).getCivID()).getPuppetOfCivID() == Game.player.iCivID)) {
            ImageManager.getImage(Game_Calendar.IMG_MANPOWER_UP).draw(oSB, this.getPosX() + this.getWidth() / 2 - ImageManager.getImage(Game_Calendar.IMG_MANPOWER_UP).getWidth() / 2 + iTranslateX, this.getPosY() + (this.getHeight() - this.getTextH()) / 2 - ImageManager.getImage(Game_Calendar.IMG_MANPOWER_UP).getHeight() / 2 + iTranslateY);
        }
        else {
    ...
    static {
        TextIcon2_Manpower.lTimeAnimation = 0L;
        TextIcon2_Manpower.animationState = 0;
    }
}
