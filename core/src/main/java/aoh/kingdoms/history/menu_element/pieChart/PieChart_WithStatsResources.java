...
public class PieChart_WithStatsResources extends PieChart_WithStats
{
    public PieChart_WithStatsResources(final int iPosX, final int iPosY, final int iWidth, final int iHeight, final PieChart_Data nPieChartData, final MenuElement_Hover menuElementHover) {
    ...
    public Image getStatImage(final int nID) {
        return ResourcesManager.resourceImages.get(nID);
    }
    
    public int getStatsImageWidth() {
        return ResourcesManager.resourceImages.get(0).getWidth();
    }
    
    public int getStatsImageHeight() {
        return ResourcesManager.resourceImages.get(0).getHeight();
    }
    ...
    protected void drawPieChart(final SpriteBatch oSB, final int iTranslateX, int iTranslateY, final boolean isActive, final boolean scrollableY, final int nPosX, final int nPosY, final int nWidth, final int nHeight, final int nWidth_LEFT) {
        ...
        if (this.isDescriptionActive || this.hideAnimation) {
            ...
            final float tempFlagScale = (CFG.TEXT_HEIGHT * this.TEXT_SCALE + CFG.PADDING) / this.getStatsImageHeight();
            for (int i = 0; i < this.pieChartData.getPieChartValuesSize(); ++i) {
                try {
                    ...
                    this.getStatImage(this.pieChartData.getPieChartValue(i).getDataID()).draw(oSB, nPosX + nWidth_LEFT + CFG.PADDING * 2 + CFG.COLOR_WIDTH + iTranslateX, nPosY + (int)(CFG.TEXT_HEIGHT * this.TEXT_SCALE + CFG.PADDING * 2) * i + CFG.PADDING * i - CFG.PADDING / 2 + (int)(CFG.TEXT_HEIGHT * this.TEXT_SCALE + CFG.PADDING * 2) / 2 - (int)(CFG.TEXT_HEIGHT * this.TEXT_SCALE / 2.0f) + iTranslateY, (int)(ResourcesManager.resourceImages.get(this.pieChartData.getPieChartValue(i).getDataID()).getWidth() * tempFlagScale), (int)(ResourcesManager.resourceImages.get(this.pieChartData.getPieChartValue(i).getDataID()).getHeight() * tempFlagScale));
                }
                catch (final IndexOutOfBoundsException ex) {
