...
public class Text_StaticBG_TaxEfficiency extends Text_StaticBG
{
    ...
    public Text_StaticBG_TaxEfficiency(final String sText, final int fontID, final int iTextPositionX, final int iPosX, final int iPosY, final int iWidth, final int iHeight, final int id) {
    ...
    public void draw(final SpriteBatch oSB, final int iTranslateX, final int iTranslateY, final boolean isActive, final boolean scrollableY) {
        ...
        if (Game.getProvince(this.id).iProvinceIncreaseTaxEfficiencySize > 0) {
            ...
            Renderer.drawBoxProgress(oSB, Images.statsRectBG, this.getPosX() + iTranslateX, this.getPosY() + iTranslateY, (int)(this.getWidth() * (1.0f - Game.getProvince(this.id).provinceIncreasTaxEfficiencyDaysLeft.get(0).daysLeft / (float)Game.getProvince(this.id).provinceIncreasTaxEfficiencyDaysLeft.get(0).investTime)), this.getHeight(), this.getWidth());
        ...
        if (this.getClickable() && this.getIsHovered() && Text_StaticBG_TaxEfficiency.animationState >= 0) {
            if (Text_StaticBG_TaxEfficiency.animationState == 0) {
                final float drawPerc = Math.min(1.0f * (CFG.currentTimeMillis - Text_StaticBG_TaxEfficiency.lTimeAnimation) / 1000.0f, 1.0f);
                ...
                if (Text_StaticBG_TaxEfficiency.lTimeAnimation < CFG.currentTimeMillis - 1000L) {
                    ++Text_StaticBG_TaxEfficiency.animationState;
                    Text_StaticBG_TaxEfficiency.lTimeAnimation = CFG.currentTimeMillis;
                }
            }
            else {
                final float drawPerc = Math.min(1.0f * (CFG.currentTimeMillis - Text_StaticBG_TaxEfficiency.lTimeAnimation) / 1000.0f, 1.0f);
                ...
                if (Text_StaticBG_TaxEfficiency.lTimeAnimation < CFG.currentTimeMillis - 1000L) {
                    Text_StaticBG_TaxEfficiency.animationState = 0;
                    Text_StaticBG_TaxEfficiency.lTimeAnimation = CFG.currentTimeMillis;
                }
            }
            oSB.setColor(Color.WHITE);
    ...
    public String getTextToDraw() {
        if (this.lastValue != Game.getProvince(this.id).getTaxEfficiencyWithBonuses()) {
            this.setText("" + CFG.getPrecision2(Game.getProvince(this.id).getTaxEfficiencyWithBonuses(), 10) + "%");
            this.lastValue = Game.getProvince(this.id).getTaxEfficiencyWithBonuses();
        }
        return this.sText;
    }
    ...
    static {
        Text_StaticBG_TaxEfficiency.lTimeAnimation = 0L;
        Text_StaticBG_TaxEfficiency.animationState = 0;
    }
}
