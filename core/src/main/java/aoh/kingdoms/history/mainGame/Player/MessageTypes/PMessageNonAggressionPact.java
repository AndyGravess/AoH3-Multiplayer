...
public class PMessageNonAggressionPact extends PMessage
{
    public PMessageNonAggressionPact(final int iFromCivID, final int iExpiresTurnID) {
        super(iFromCivID, iExpiresTurnID, 0);
    }
    
    @Override
    public void actionClick() {
        if (Game.menuManager.getVisibleInGame_PopUp() && MenuManager.IN_GAME_POP_UP_MENU_ID == 31 && InGame_MessageNonAggressionPact.key.equals(this.key)) {
            Game.menuManager.setVisibleInGame_PopUp(false);
        }
        else {
            Game.menuManager.rebuildInGame_MessageNonAggressionPact(this.key);
        }
    }
    
    @Override
    public void onAccept() {
        DiplomacyManager.acceptNonAggressionOffer(this.fromCivID, Game.player.iCivID);
        ProvinceDraw.addDiplomacyLines(Game.getCiv(Game.player.iCivID).getCapitalProvinceID(), Game.getCiv(this.fromCivID).getCapitalProvinceID(), DiplomacyManager.COLOR_NON_AGGRESSION_PACT);
    }
    ...
    public MenuElement_Hover buildElementHover() {
        ...
        nData.add(new MenuElement_HoverElement_Type_TextTitle_BG(Game.lang.get("NonAggressionPact"), CFG.FONT_BOLD, Colors.HOVER_GOLD));
