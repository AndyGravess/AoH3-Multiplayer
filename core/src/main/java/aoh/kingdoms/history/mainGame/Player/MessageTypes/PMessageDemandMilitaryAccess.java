...
public class PMessageDemandMilitaryAccess extends PMessage
{
    public PMessageDemandMilitaryAccess(final int iFromCivID, final int iExpiresTurnID) {
        super(iFromCivID, iExpiresTurnID, 0);
    }
    
    @Override
    public void actionClick() {
        if (Game.menuManager.getVisibleInGame_PopUp() && MenuManager.IN_GAME_POP_UP_MENU_ID == 32 && InGame_MessageDemandsMilitaryAccess.key.equals(this.key)) {
            Game.menuManager.setVisibleInGame_PopUp(false);
        }
        else {
            Game.menuManager.rebuildInGame_MessageDemandsMilitaryAccess(this.key);
        }
    }
    
    @Override
    public void onAccept() {
        DiplomacyManager.acceptDemandMilitaryAccess(this.fromCivID, Game.player.iCivID);
        ProvinceDraw.addDiplomacyLines(Game.getCiv(Game.player.iCivID).getCapitalProvinceID(), Game.getCiv(this.fromCivID).getCapitalProvinceID(), DiplomacyManager.COLOR_MILITARY_ACCESS);
    }
    ...
    public MenuElement_Hover buildElementHover() {
        ...
        nData.add(new MenuElement_HoverElement_Type_TextTitle_BG(Game.lang.get("CivAWantsToHaveMilitaryAccessToOurTerritory", Game.getCiv(this.fromCivID).getCivName()), CFG.FONT_BOLD, Colors.HOVER_GOLD));
