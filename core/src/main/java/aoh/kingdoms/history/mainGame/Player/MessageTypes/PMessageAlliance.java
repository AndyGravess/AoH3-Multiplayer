...
public class PMessageAlliance extends PMessage
{
    public PMessageAlliance(final int iFromCivID, final int iExpiresTurnID) {
        super(iFromCivID, iExpiresTurnID, 0);
    }
    
    @Override
    public void actionClick() {
        if (Game.menuManager.getVisibleInGame_PopUp() && MenuManager.IN_GAME_POP_UP_MENU_ID == 29 && InGame_MessageAlliance.key.equals(this.key)) {
            Game.menuManager.setVisibleInGame_PopUp(false);
        }
        else {
            Game.menuManager.rebuildInGame_MessageAlliance(this.key);
        }
    }
    
    @Override
    public void onAccept() {
        DiplomacyManager.acceptAllianceOffer(this.fromCivID, Game.player.iCivID);
        DesktopLauncher.SMS("AllyOK: " + this.fromCivID + " " + Game.player.iCivID);
